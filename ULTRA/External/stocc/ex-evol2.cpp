...
//-----------------------------------------------------------------------------
void SplitIntoGenotypes(int32_t *genotypes, int32_t *genes,
                        StochasticLib1 &stoc) {
  // split gene pool into (a,a), (a,b) and (b,b) genotypes
  // parameters:
  // genotypes:       output. array with 3 places.
  //                  gets number of (a,a), (a,b), and (b,b) genotypes,
  //                  respectively
...
int BreedingSuccess(double *ChildGenotypes, int32_t *FathersGenotypes,
  ...
  // 2 = (b,b)
  // Parameters:
  // ChildGenotypes:   Results. The relative probability of each genotype in
  // children. FathersGenotypes: The number of male parents of each genotype
...
int main() {
  ...
  int32_t CarryingCapacity =
      100000;                       // maximum number of individuals in habitat
  double CapacityDeviation = 1000;  // standard deviation of max individuals
  double SelectionCoefficient =
      0.1;                 // coefficient of selection in favor of B over A
  ...
  // Generation loop
  //---------------------------------------------------------------------------
  for (GenerationNumber = 1; GenerationNumber <= Generations;
       GenerationNumber++) {
    // mutation
    ...
    i = BreedingSuccess(ChildProb, GenotypesMales, GenotypesFemales,
                        GenoSelection, Mul);
    if (!i) {
    ...
    if (GenerationNumber % OutputPeriod == 0) {
      printf("\n  %8li   %8li  %8li", GenerationNumber, ParentGenePool[0],
             ParentGenePool[1]);
