...
int32_t mlist[] = {50, 30, 50, 80};  // number of balls of each color

double wlist[] = {1, 6, 3, 2};  // weight of each color
...
int main() {
  ...
  // print message
  printf(
      "\nsampling from multivariate Fisher's noncentral hypergeometric "
      "distribution...");
  ...
  for (i = 0, cmb = 1.; i < colors; i++)
    if (var1[i] > 0.) cmb *= 4.5 * sqrt(var1[i]);
  if (cmb < 1E9) {
  ...
  // print table heading
  printf(
      "\n                     sampled    approx     exact   sampled    "
      "approx     exact");
  printf(
      "\n   balls    weight      mean      mean      mean  variance  "
      "variance  variance");
  ...
  for (i = 0; i < colors; i++) {
    ...
    // output table line
    printf("\n%8li %9.3f %9.3f %9.3f %9.3f %9.3f %9.3f %9.3f", mlist[i],
           wlist[i], mean0, mu1[i], mu2[i], var0, var1[i], var2[i]);
  }
