...
double Erf(double x) {
  ...
  if (x < 2.4) {
    ...
    double a, f;
    int n =
        int(2.25f * x * x - 23.4f * x + 60.84f);  // predict expansion degree
    if (n < 1) n = 1;
    a = 0.5 * n;
    f = x;
...
void CWalleniusNCHypergeometric::SetParameters(int32_t n_, int32_t m_,
  ...
  xmin = m + n - N;
  if (xmin < 0) xmin = 0;  // calculate xmin
  xmax = n;
  if (xmax > m) xmax = m;           // calculate xmax
...
double CWalleniusNCHypergeometric::mean(void) {
  ...
  if (omega == 0.) {
    if (n > N - m)
      FatalError(
          "Not enough items with nonzero weight in "
  ...
  if (omega > 1.) {
    do {  // Newton Raphson iteration
      ...
      if (++iter > 40) {
        FatalError(
            "Search for mean failed in function "
            "CWalleniusNCHypergeometric::mean");
      }
...
double CWalleniusNCHypergeometric::moments(double *mean_, double *var_) {
  ...
  for (x = xm; x <= xmax; x++) {
    y = probability(x);
    x1 =
        x - xm;  // subtract approximate mean to avoid loss of precision in sums
    sy += y;
    sxy += x1 * y;
    sxxy += x1 * x1 * y;
...
void CWalleniusNCHypergeometric::findpars() {
  ...
  do {
    ...
    if (++j == 70)
      FatalError(
          "convergence problem searching for r in function "
  ...
  if (phi2d >= 0.) {
    FatalError(
        "peak width undefined in function "
        "CWalleniusNCHypergeometric::findpars");
    /* wr = r = 0.; */
  } else {
    wr = sqrt(-phi2d);
...
double CWalleniusNCHypergeometric::recursive() {
  ...
  for (nu = 1; nu <= n; nu++) {
    ...
    for (xi = x2; xi >= x1; xi--) {  // backwards loop
      ...
      y1 =
          p1[xi - 1];  // (warning: pointer alias, can't swap instruction order)
      p2[xi] = y;
    }
    p1 = p2;
  }
...
double CWalleniusNCHypergeometric::laplace() {
  ...
  while (w * NumSDev[PrecisionIndex] > 0.3) {
    ...
    if (PrecisionIndex == 0) {
      FatalError(
          "Laplace method failed. Peak width too high in function "
...
double CWalleniusNCHypergeometric::search_inflect(double t_from, double t_to) {
  ...
  do {
    ...
    if (++iter > 20)
      FatalError(
          "Search for inflection point failed in function "
...
int32_t CWalleniusNCHypergeometric::MakeTable(double *table, int32_t MaxLength,
  ...
  area =
      double(n) * LengthNeeded;  // Estimate calculation time for table method
...
void CMultiWalleniusNCHypergeometric::SetParameters(int32_t n_, int32_t *m_,
  ...
  for (N = N1 = 0, i = 0; i < colors; i++) {
    if (m[i] < 0 || omega[i] < 0)
      FatalError(
          "Parameter negative in constructor for "
  ...
  if (N1 < n)
    FatalError(
        "Not enough items with nonzero weight in constructor for "
        "CMultiWalleniusNCHypergeometric");
}
...
double CMultiWalleniusNCHypergeometric::probability(int32_t *x_) {
  ...
  if (xsum != n) {
    FatalError(
        "sum of x values not equal to n in function "
