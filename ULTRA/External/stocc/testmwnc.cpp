...
int main() {
  ...
  // print message
  printf(
      "\nsampling from multivariate Wallenius noncentral hypergeometric "
      "distribution...");
  ...
  for (i = 0; i < colors; i++) {
    ...
    // output table line
    printf("\n%8li %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f", mlist[i],
           wlist[i], mean1, mu1[i], mu2[i], var1, var[i]);
  }
