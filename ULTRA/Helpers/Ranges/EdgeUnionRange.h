...
class EdgeUnionRange {
 ...
 public:
    ...
    inline Iterator &validate() noexcept {
      if (edgeA != outer->rangeA.end()) {
        if ((edgeB != outer->rangeB.end()) &&
            (outer->graphA->get(ToVertex, *edgeA) >=
             outer->graphB->get(ToVertex, *edgeB))) {
          useB = true;
          if (outer->graphA->get(ToVertex, *edgeA) ==
              outer->graphB->get(ToVertex, *edgeB)) {
            useA = true;
          } else {
            useA = false;
  ...
  inline Edge back() const noexcept {
    AssertMsg(!empty(), "Range is empty!");
    if ((!rangeA.empty()) &&
        ((rangeB.empty()) || (graphA->get(ToVertex, rangeA.back()) >=
                              graphB->get(ToVertex, rangeB.back())))) {
      return *(rangeA.back());
    } else {
