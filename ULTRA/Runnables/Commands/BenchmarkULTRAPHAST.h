...
class RunOneToAllDijkstraCSAQueriesToVertices : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    csaData.printInfo();
    CSA::OneToAllDijkstraCSA<CSA::TransferGraph, true, CSA::AggregateProfiler>
        algorithm(csaData);

    const size_t n = getParameter<size_t>("Number of queries");
    const std::vector<OneToAllQuery> queries =
        generateRandomOneToAllQueries(csaData.transferGraph.numVertices(), n);
...
class RunOneToManyDijkstraCSAQueriesToStops : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    CSA::OneToAllDijkstraCSA<CHGraph, true, CSA::AggregateProfiler> algorithm(
        csaData, upwardGraph, Weight);
...
inline IndexedSet<false, Vertex> getTargetSet(const DATA &data,
                                              const size_t numVertices,
                                              const bool vertices) noexcept {
  if (vertices) {
...
class RunUPCSAQueries : public ParameterizedCommand {
 ...
 private:
  ...
  inline void run() const noexcept {
    ...
    const size_t n = getParameter<size_t>("Number of queries");
    const std::vector<OneToAllQuery> queries =
        generateRandomOneToAllQueries(ch.numVertices(), n);
    ...
    std::cout << "\tUpward sweep graph vertices: "
              << String::prettyInt(algorithm.getUpwardSweepGraphVertices())
              << std::endl;
    std::cout << "\tUpward sweep graph edges: "
              << String::prettyInt(algorithm.getUpwardSweepGraphEdges())
              << std::endl;
    std::cout << "\tStop graph vertices: "
              << String::prettyInt(algorithm.getStopGraphVertices())
              << std::endl;
    std::cout << "\tStop graph edges: "
              << String::prettyInt(algorithm.getStopGraphEdges()) << std::endl;
    std::cout << "\tTarget graph vertices: "
              << String::prettyInt(algorithm.getTargetGraphVertices())
              << std::endl;
    std::cout << "\tTarget graph edges: "
              << String::prettyInt(algorithm.getTargetGraphEdges())
              << std::endl;
    std::cout << std::endl;
...
class RunOneToAllDijkstraRAPTORQueriesToVertices : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    RAPTOR::OneToAllDijkstraRAPTOR<RAPTOR::DijkstraInitialTransfers,
                                   RAPTOR::AggregateProfiler>
        algorithm(raptorData, raptorData.transferGraph,
                  reverseRaptorData.transferGraph);

    const size_t n = getParameter<size_t>("Number of queries");
    const std::vector<OneToAllQuery> queries = generateRandomOneToAllQueries(
        raptorData.transferGraph.numVertices(), n);
...
class RunUPRAPTORQueries : public ParameterizedCommand {
 public:
  RunUPRAPTORQueries(BasicShell &shell)
      : ParameterizedCommand(
            ...
            "Runs the given number of random UP-RAPTOR queries.") {
    addParameter("RAPTOR data");
 ...
 private:
  ...
  inline void run() const noexcept {
    ...
    using UPRAPTOR =
        RAPTOR::UPRAPTOR<GROUPED_ROUNDS, RAPTOR::AggregateProfiler>;
    const bool reorder = getParameter<bool>("Reorder network?");
    Timer timer;
...
class RunUPTBQueries : public ParameterizedCommand {
 ...
 private:
  ...
  inline void run() const noexcept {
    ...
    IndexedSet<false, Vertex> targetSet =
        getTargetSet(tripBasedData.raptorData, ch.numVertices(),
                     getParameter("Targets") == "Vertices");

    using UPTB =
        TripBased::UPQuery<GROUPED_ROUNDS, TripBased::AggregateProfiler>;
    const bool reorder = getParameter<bool>("Reorder network?");
    Timer timer;
...
using WitnessSearch =
    CH::BidirectionalWitnessSearch<CHCoreGraph, CH::FullProfiler, 200>;
...
struct CHData {
  ...
  CHData(const CH::CH &&ch, const TransferGraph &originalGraph)
      : ch(std::move(ch)), numCoreVertices(0) {
    ...
    for (const Vertex vertex : tempCore.vertices()) {
      if (ch.isCoreVertex(vertex)) {
        ...
        for (const Edge edge : ch.forward.edgesFrom(vertex)) {
          tempCore.addEdge(vertex, ch.forward.get(ToVertex, edge))
...
template <typename KEY_FUNCTION, typename STOP_CRITERION>
inline CHData buildCH(const TransferGraph &originalGraph,
                      const KEY_FUNCTION &keyFunction,
...
inline CHData buildCoreCH(const RAPTOR::Data &data,
                          ...
                          const size_t coreDegree) noexcept {
  ...
  PartialKey keyFunction(isCoreContractable, data.transferGraph.numVertices(),
                         getGreedyKey());
...
inline CH::CH buildULTRACH(const RAPTOR::Data &data,
  ...
  std::vector<size_t> firstContractableRound(data.transferGraph.numVertices(),
                                             0);
  for (const Vertex target : targets) {
    firstContractableRound[target] = targetRound;
  }
  for (const StopId stop : data.stops()) {
    firstContractableRound[stop] =
        std::max(firstContractableRound[stop], stopRound);
  }
...
inline std::vector<Vertex> generateTargetSet(const TransferGraph &graph,
                                             const size_t ballSize,
                                             const size_t numTargets) noexcept {
 ...
 public:
  CreateBallTargetSets(BasicShell &shell)
      : ParameterizedCommand(
            ...
            "Creates random ball target sets for the given network.") {
    addParameter("RAPTOR data");
  ...
  virtual void execute() noexcept {
    ...
    const size_t ballSize = std::min(raptorData.transferGraph.numVertices(),
                                     size_t(numTargets * ballSizeFactor));
    ...
    for (size_t i = 0; i < numTargetSets; i++) {
      targets.emplace_back(
          generateTargetSet(raptorData.transferGraph, ballSize, numTargets));
    }
 ...
 private:
  struct BenchmarkData {
    ...
    inline friend std::ostream &operator<<(std::ostream &out,
                                           const BenchmarkData &data) noexcept {
      out << "Time: " << String::musToString(data.buildTime / data.count)
          << std::endl;
      out << "Core vertices: "
          << String::prettyDouble(data.coreVertices / data.count) << std::endl;
      out << "Core edges: " << String::prettyDouble(data.coreEdges / data.count)
          << std::endl;
      out << "CH edges: " << String::prettyDouble(data.chEdges / data.count)
          << std::endl;
      return out;
    }
 ...
 public:
  BuildUPCHForTargetSets(BasicShell &shell)
      : ParameterizedCommand(
            ...
            "Builds ULTRA-PHAST-CHs for the given target sets.") {
    addParameter("RAPTOR data");
    addParameter("Targets file");
  ...
  virtual void execute() noexcept {
    ...
    for (size_t i = 0; i < targets.size(); i++) {
      timer.restart();
      const CH::CH ch =
          buildULTRACH(raptorData, targets[i], stopFactor, targetFactor);
 ...
 public:
  RunOneToManyDijkstraCSAQueriesToBall(BasicShell &shell)
      : ParameterizedCommand(shell, "runOneToManyDijkstraCSAQueriesToBall",
                             "Runs the given number of random Dijkstra-CSA "
                             "queries to given ball target sets.") {
    addParameter("CSA data");
    addParameter("Targets file");
    addParameter("CH data");
    addParameter("Number of queries");
  ...
  virtual void execute() noexcept {
    ...
    using Algorithm =
        CSA::OneToAllDijkstraCSA<CHGraph, true, CSA::AggregateProfiler>;

    CSA::AggregateProfiler profiler;
    profiler.registerPhases({CSA::PHASE_CLEAR, CSA::PHASE_INITIALIZATION,
                             CSA::PHASE_CONNECTION_SCAN,
                             CSA::PHASE_FINAL_TRANSFERS});
    profiler.registerMetrics({CSA::METRIC_CONNECTIONS, CSA::METRIC_EDGES,
                              CSA::METRIC_STOPS_BY_TRIP,
...
struct SetupBenchmarkData {
  ...
  inline friend std::ostream &operator<<(
    ...
    out << "\tTime: " << String::musToString(data.buildTime / data.count)
        << std::endl;
    out << "\tUpward sweep graph vertices: "
        << String::prettyDouble(data.upwardSweepGraphVertices / data.count)
        << std::endl;
    out << "\tUpward sweep graph edges: "
        << String::prettyDouble(data.upwardSweepGraphEdges / data.count)
        << std::endl;
    out << "\tStop graph vertices: "
        << String::prettyDouble(data.stopGraphVertices / data.count)
        << std::endl;
    out << "\tStop graph edges: "
        << String::prettyDouble(data.stopGraphEdges / data.count) << std::endl;
    out << "\tTarget graph vertices: "
        << String::prettyDouble(data.targetGraphVertices / data.count)
        << std::endl;
    out << "\tTarget graph edges: "
        << String::prettyDouble(data.targetGraphEdges / data.count)
        << std::endl;
    return out;
  }
...
class RunUPCSAQueriesToBall : public ParameterizedCommand {
 ...
 private:
  ...
  inline void run() const noexcept {
    ...
    const size_t n = getParameter<size_t>("Number of queries");
    const std::vector<OneToAllQuery> queries =
        generateRandomOneToAllQueries(dummyCH.numVertices(), n);

    const bool reorder = getParameter<bool>("Reorder network?");
    const bool useDFSOrder = getParameter("Order") == "DFS";

    using Algorithm = CSA::UPCSA<USE_STOP_BUCKETS, USE_TARGET_BUCKETS, true,
                                 CSA::AggregateProfiler>;

    CSA::AggregateProfiler profiler;
    profiler.registerPhases({CSA::PHASE_CLEAR, CSA::PHASE_INITIALIZATION,
                             CSA::PHASE_CONNECTION_SCAN,
                             CSA::PHASE_UPWARD_SWEEP,
                             CSA::PHASE_DOWNWARD_SEARCH});
    profiler.registerMetrics({CSA::METRIC_CONNECTIONS, CSA::METRIC_EDGES,
                              CSA::METRIC_STOPS_BY_TRIP,
                              CSA::METRIC_STOPS_BY_TRANSFER});
 ...
 public:
  ...
  virtual void execute() noexcept {
    ...
    profiler.registerExtraRounds(
        {RAPTOR::EXTRA_ROUND_CLEAR, RAPTOR::EXTRA_ROUND_INITIALIZATION});
    profiler.registerPhases({RAPTOR::PHASE_INITIALIZATION,
                             RAPTOR::PHASE_COLLECT, RAPTOR::PHASE_SCAN,
                             RAPTOR::PHASE_TRANSFERS});
    profiler.registerMetrics(
        {RAPTOR::METRIC_ROUTES, RAPTOR::METRIC_ROUTE_SEGMENTS,
         RAPTOR::METRIC_VERTICES, RAPTOR::METRIC_EDGES,
         RAPTOR::METRIC_STOPS_BY_TRIP, RAPTOR::METRIC_STOPS_BY_TRANSFER});
...
class RunUPRAPTORQueriesToBall : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    RAPTOR::AggregateProfiler profiler;
    profiler.registerExtraRounds({RAPTOR::EXTRA_ROUND_CLEAR,
                                  RAPTOR::EXTRA_ROUND_INITIALIZATION,
                                  RAPTOR::EXTRA_ROUND_FINAL_TRANSFERS});
    profiler.registerPhases({RAPTOR::PHASE_INITIALIZATION,
                             RAPTOR::PHASE_COLLECT, RAPTOR::PHASE_SCAN,
                             RAPTOR::PHASE_TRANSFERS,
                             RAPTOR::PHASE_FINAL_TRANSFERS});
    profiler.registerMetrics(
        {RAPTOR::METRIC_ROUTES, RAPTOR::METRIC_ROUTE_SEGMENTS,
         RAPTOR::METRIC_EDGES, RAPTOR::METRIC_STOPS_BY_TRIP,
