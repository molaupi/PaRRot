...
#include "../../Algorithms/KaRRi/CH/CH.h"
...
inline constexpr auto RIDESHARING_FILENAME =
    "../Data/StuttgartCity/RideData/veh100_req200_serv28800_alpha1.1/rideData";
inline constexpr auto ROADNETWORK_FILENAME =
    "../Data/StuttgartCity/Car/Extended/roadGraph";
inline constexpr auto ROADNETWORKCH_FILENAME =
    "../Data/StuttgartCity/Car/CH/ch";
inline constexpr auto RAPTOR_FILENAME =
    "../Data/StuttgartCity/Raptor/raptor.binary";
inline constexpr auto WALKINGCH_FILENAME =
    "../Data/StuttgartCity/Walking/CoreCH/ch";
inline constexpr auto DISTANCEMATRIX_FILENAME =
    "../Data/StuttgartCity/RideData/distanceMatrix";
...
inline std::vector<Loud::Vehicle> generateVehicles(
  ...
  for (int i = 0; i < numberOfVehicles; i++) {
    ...
    startServTime =
        (std::max(timeInterval - servTime, 0) / (numberOfVehicles - 1)) *
        i;  //(timeInterval / numberOfVehicles) * i;
...
inline std::vector<Loud::Vehicle> readVehicleDataFromFile(
  ...
  std::vector<Loud::Vehicle> fleet;
  int loc, capacity, startServTime, endServTime;
  io::CSVReader<4, io::trim_chars<>> vehFileReader(vehicleFileName +
                                                   ".vehicles.csv");
...
class RunRideRaptor : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    const std::string distanceMatrixFileName =
        getParameter("distance matrix file");
    ...
    std::vector<Loud::Vehicle> fleet =
        readVehicleDataFromFile(ridesharingFileName);
...
class RunPreprocessing : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    const auto numberOfInsertedRequests =
        getParameter<int>("inserted request number");
    ...
    std::vector<Loud::Request> requests =
        generateRequests(inputGraph.numEdges(), numberOfRequests, timeInterval);
    std::cout << "done.\n";
    ...
    const std::string identifier =
        "veh" + std::to_string(numberOfVehicles) + "_req" +
        std::to_string(insertionCount) + "_serv" + std::to_string(servTime) +
        "_alpha" + std::to_string(parameter.alpha).substr(0, 3);
    const std::string outputFileName =
        getParameter("output folder") + "/" + identifier + "/rideData";
 ...
 private:
  inline void run(RIDERAPTOR::Data &data) const noexcept {
    ...
    const std::vector<RideStopQuery> queries =
        generateRandomRideStopQueries(locationRange, n, data.raptorData);
    ...
    algorithm.getProfiler().writeToFile(
        getParameter("ridesharing file") + ".queries" + std::to_string(n) +
            "_flags" + std::to_string(int(flags.UseTimePruning)) +
            std::to_string(int(flags.UseLeewayPruning)) +
            std::to_string(int(flags.UseApproximation)) +
            std::to_string(int(flags.UseDistanceMatrix)) +
            std::to_string(int(restrictToStops)) + ".csv",
        names, values);
  }
};
...
class ExtendGraph : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    TransferGraph networkExtended =
        RIDERAPTOR::getOverheadGraph(raptorData, raptorData.transferGraph);
...
class RunMultiplePreprocessing : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    const auto routeLengthInterval =
        getParameter<double>("route length interval");
    const auto insertedRequestInterval =
        int((numberOfVehicles * routeLengthInterval) / 2);
    const auto timeInterval = getParameter<int>("time interval");
    const auto servTime = getParameter<int>("service time");
    RAPTOR::Data raptorData = RAPTOR::Data::FromBinary(raptorFileName);
    InputGraph inputGraph(networkFileName);
    ...
    for (int i = 0; i < numIterations; i++) {
      ...
      std::ofstream outputFile(outputFileName + "_veh" +
                               std::to_string(numberOfVehicles) + "_alpha" +
                               std::to_string(parameter.alpha).substr(0, 3) +
      ...
      for (const auto &req : requests) {
        ...
        if (buildRideTransfer &&
            (insertionCount % insertedRequestInterval == 0)) {
  ...
  inline void buildRideTransferGraph(RAPTOR::Data &raptorData, Dispatcher &disp,
    ...
    outputFile << double(numberOfRequests * 2 + disp.fleet.size()) /
                      double(disp.fleet.size())
               << ",";
...
class RunQueriesOnMultiplePreprocessings : public ParameterizedCommand {
 public:
  ...
  virtual void execute() noexcept {
    ...
    const size_t n = getParameter<size_t>("number of queries");
    const bool restrictToStops = getParameter<bool>("restrict to stops?");
    const size_t locationRange =
        restrictToStops ? raptorData.numberOfStops() : inputGraph.numEdges();
    const std::vector<RideStopQuery> queries =
        generateRandomRideStopQueries(locationRange, n);
    std::cout << "done.\n";
    ...
    std::ofstream outputFile(
        outputFileName + "_veh" + std::to_string(numberOfVehicles) + "_alpha" +
        std::to_string(parameter.alpha).substr(0, 3) + "_flags" +
        std::to_string(int(flags.UseTimePruning)) +
        std::to_string(int(flags.UseLeewayPruning)) +
        std::to_string(int(flags.UseApproximation)) +
        std::to_string(int(flags.UseDistanceMatrix)) +
        std::to_string(int(restrictToStops)) + ".csv");
  ...
  inline void run(RIDERAPTOR::Data &data) const noexcept {
    ...
    for (int depTime = 0; depTime <= 86400; depTime += interval) {
      ...
      for (const RideStopQuery &query : queries) {
        ...
        numRideJourneys +=
            usesRide(algorithm.getEarliestJourney(StopId(query.target)));
        ...
        rideRaptorTripTime +=
            (algorithm.getEarliestArrivalTime(StopId(query.target)) - depTime);
        raptorTripTime +=
            (raptor.getEarliestArrivalTime(StopId(query.target)) - depTime);
      }
