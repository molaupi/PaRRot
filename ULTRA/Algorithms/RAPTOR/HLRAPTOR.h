...
template <typename PROFILER = NoProfiler> class HLRAPTOR {
...
public:
  HLRAPTOR(const Data &data, const InitialTransferGraph &outHubGraph,
           const InitialTransferGraph &inHubGraph,
      ...
      : data(data), outHubs(outHubGraph), inHubs(inHubGraph),
        ...
        lastTarget(Vertex(0)), sourceDepartureTime(never),
  ...
  getRouteDescription(const Vertex vertex) const noexcept {
    const StopId target =
        (vertex == targetVertex) ? (targetStop) : (StopId(vertex));
  ...
  inline int getArrivalTime(const Vertex vertex,
    ...
    AssertMsg(rounds[numberOfTrips][target].arrivalTime < never,
              "No label found for stop " << target << " in round "
                                         << numberOfTrips << "!");
...
private:
  ...
  inline void relaxInitialTransfers(const int sourceDepartureTime) noexcept {
    ...
    for (const Edge edge : inHubs.edgesFrom(targetVertex)) {
      transferDistanceToTarget[inHubs.get(ToVertex, edge)] =
          inHubs.get(TravelTime, edge);
    }
    ...
    for (const Edge edge : outHubs.edgesFrom(sourceVertex)) {
      ...
      const int arrivalTime =
          sourceDepartureTime + outHubs.get(TravelTime, edge);
  ...
  inline void relaxIntermediateTransfers() noexcept {
    ...
    for (const StopId stop : stopsUpdatedByRoute) {
      ...
      for (const Edge edge : outHubs.edgesFrom(stop)) {
        ...
        profiler.countMetric(METRIC_EDGES);
        const int arrivalTime =
            earliestArrivalTime + outHubs.get(TravelTime, edge);
        if (earliestArrival[targetVertex] <= arrivalTime)
          break;
  ...
  inline void relaxTransfersFromHubs() noexcept {
    ...
    for (const Vertex hub : updatedHubs) {
      for (const Edge edge : reverseInHubs.edgesFrom(hub)) {
        ...
        profiler.countMetric(METRIC_EDGES);
        const int arrivalTime =
            earliestArrival[hub] + reverseInHubs.get(TravelTime, edge);
        if (earliestArrival[targetVertex] <= arrivalTime)
          break;
      ...
      if (transferDistanceToTarget[hub] != INFTY) {
        profiler.countMetric(METRIC_EDGES);
        const int arrivalTime =
            earliestArrival[hub] + transferDistanceToTarget[hub];
        arrivalByTransfer(targetVertex, arrivalTime,
                          hubParentLabels[hub].parent,
                          hubParentLabels[hub].parentDepartureTime);
      }
    }
  }
  ...
  inline void arrivalAtHub(const Vertex hub, const int time,
                           const Vertex parent,
  ...
  inline void arrivalByTransfer(const Vertex vertex, const int time,
                                const Vertex parent,
                                const int parentDepartureTime) noexcept {
    AssertMsg(data.isStop(vertex) || vertex == targetVertex,
              "Stop " << vertex << " is out of range!");
