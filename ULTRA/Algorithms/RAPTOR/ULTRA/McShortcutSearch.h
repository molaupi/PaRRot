...
public:
  ...
  using KeyType =
      Meta::IF<UseArrivalKey, std::tuple<int, int>, std::tuple<int, int, int>>;
...
public:
  McShortcutSearch(const Data &data, DynamicTransferGraph &shortcutGraph,
                   const int intermediateWitnessTransferLimit,
                   const int finalWitnessTransferLimit)
      : data(data), shortcutGraph(shortcutGraph),
        stationOfStop(data.numberOfStops()), sourceStation(),
        sourceDepartureTime(0), shortcutCandidatesInQueue(0),
...
private:
      ...
      while (stopIndex < tripSize - 1) {
        if (stopsUpdatedByTransfer.contains(stop)) {
          const int walkingDistance =
              directTransferArrivalLabels[stop].arrivalTime;
  ...
  inline void scanRoutes1() noexcept {
    ...
    for (const RouteId route : routesServingUpdatedStops.getKeys()) {
      ...
      while (tripIterator.hasFurtherStops()) {
        ...
        const int walkingDistance =
            directTransferArrivalLabels[parent].arrivalTime;
        const bool isCandidate = stationOfStop[parent].representative ==
                                 sourceStation.representative;
  ...
  inline OneTripLabel getWalkingLabel(const Vertex vertex) noexcept {
    ...
    return OneTripLabel(walkingDistance == INFTY
                            ? INFTY
                            : sourceDepartureTime + walkingDistance,
                        walkingDistance);
  }
