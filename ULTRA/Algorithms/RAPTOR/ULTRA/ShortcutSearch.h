...
public:
  ShortcutSearch(const Data &data, DynamicTransferGraph &shortcutGraph,
      ...
      : data(data), shortcutGraph(shortcutGraph),
        ...
        optimalCandidates(0), routesServingUpdatedStops(data.numberOfRoutes()),
...
private:
  ...
  template <int CURRENT> inline void scanRoutes() noexcept {
      ...
      for (Edge edge : data.transferGraph.edgesFrom(currentVertex)) {
        ...
        // Candidates may dominate equivalent labels from previous iterations
          ...
          const bool isCandidate =
              oneTripTransferParent[currentVertex] != noStop;
  ...
  inline bool isShortcutCandidate(const Vertex vertex) const noexcept {
    return oneTripArrivalLabels[vertex].isOnHeap() &&
           data.isStop(oneTripTransferParent[vertex]);
  }
