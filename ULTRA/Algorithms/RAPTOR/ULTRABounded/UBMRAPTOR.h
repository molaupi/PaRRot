...
template <typename PROFILER = NoProfiler> class UBMRAPTOR {
...
public:
  UBMRAPTOR(const Data &data, const Data &backwardData, const CH::CH &chData,
      ...
      : data(data), initialTransfers(chData, FORWARD, data.numberOfStops()),
        ...
        backwardPruningRAPTOR(backwardData, initialTransfers,
...
private:
  ...
  inline void arrival(const StopId stop, const Label &label,
    ...
    AssertMsg(bestLabels[stop].dominates(currentRound()[stop]),
