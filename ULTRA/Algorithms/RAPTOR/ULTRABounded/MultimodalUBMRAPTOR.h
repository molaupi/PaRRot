...
public:
  ...
  inline static constexpr bool AddIntermediateOverhead =
      ADD_INTERMEDIATE_OVERHEAD;
  ...
  using Type =
      MultimodalUBMRAPTOR<NumTransferModes, Profiler, InitialTransferType,
                          AddIntermediateOverhead>;
...
private:
  MultimodalUBMRAPTOR(const MultimodalData &data,
      ...
      : data(data), transitiveInitialTransfers(data.raptorData.transferGraph,
                                               backwardTransitiveGraph),
        multimodalInitialTransfers(transitiveInitialTransfers, initialTransfers,
                                   data.modes, numVertices,
                                   data.raptorData.numberOfStops()),
        profiler(profilerTemplate),
        forwardPruningRAPTOR(forwardPruningData, multimodalInitialTransfers,
                             profiler),
        backwardPruningRAPTOR(backwardPruningData, multimodalInitialTransfers,
                              forwardPruningRAPTOR, profiler),
        bestBagByRoute(data.raptorData.numberOfStops() + 1),
        bestBagByTransfer(data.raptorData.numberOfStops() + 1), maxTrips(-1),
...
public:
  template <typename T = CHGraph, typename = std::enable_if_t<
                                      Meta::Equals<T, CHGraph>() &&
  ...
  MultimodalUBMRAPTOR(const MultimodalData &data,
                      ...
                      const Data &backwardPruningData,
                      const TransferGraph &backwardTransitiveGraph,
                      const std::vector<TransferGraph> &backwardGraphs,
                      const Profiler &profilerTemplate = Profiler())
      : MultimodalUBMRAPTOR(data, forwardPruningData, backwardPruningData,
                            backwardTransitiveGraph,
                            backwardGraphs[0].numVertices(), profilerTemplate) {
    AssertMsg(backwardGraphs.size() == NumTransferModes,
              "Wrong number of modes");
    for (size_t i = 0; i < backwardGraphs.size(); i++) {
      initialTransfers.emplace_back(getTransferGraph(i), backwardGraphs[i],
                                    data.raptorData.numberOfStops(),
                                    TravelTime);
    }
  }
  ...
  inline void run(const Vertex source, const int departureTime,
    ...
    sourceVertex = source;
    targetVertex = target;
    targetStop = data.raptorData.isStop(target)
                     ? StopId(target)
                     : StopId(data.raptorData.numberOfStops());
  ...
  inline void verify(const double arrivalSlack, const double tripSlack,
    ...
    const std::vector<ArrivalLabel> &anchorLabels =
        forwardPruningRAPTOR.getAnchorLabels();
...
private:
  ...
  inline void relaxIntermediateTransfers(const size_t mode) noexcept {
    for (const StopId stop : stopsUpdatedByRoute) {
      ...
      for (const Edge edge : getTransferGraph(mode).edgesFrom(stop)) {
        profiler.countMetric(METRIC_EDGES);
        const StopId toStop =
            StopId(getTransferGraph(mode).get(ToVertex, edge));
  ...
  inline int getIntermediateTravelTime(const size_t mode,
  ...
  inline void arrivalByRoute(const StopId stop, const Label &label) noexcept {
    ...
    if (-backwardPruningRAPTOR.getArrivalTime(
...
private:
  ...
  Profiler profiler;
  ForwardPruningULTRARAPTOR<Profiler, MultimodalInitialTransferType>
      forwardPruningRAPTOR;
  BackwardPruningULTRARAPTOR<Profiler, MultimodalInitialTransferType>
