...
class RideRAPTOR {
 public:
  ...
  static constexpr bool SeparateRouteAndTransferEntries =
      !Transitive | UseMinTransferTimes;
  ...
  using Type =
      RideRAPTOR<TargetPruning, Profiler, Transitive, UseMinTransferTimes>;
 ...
 public:
  ...
  inline void run(const Edge source, const int departureTime, const Edge target,
    ...
    profiler.startPhase();
    if (!data.raptorData.isStop(targetStop) ||
        !data.raptorData.isStop(sourceStop)) {
 ...
 private:
  ...
  inline void relaxInitialWalkingTransfer() {
    for (const Vertex stop : initialWalkingTransfers.getForwardPOIs()) {
      if (stop == targetStop || stop == sourceStop) continue;
      AssertMsg(data.raptorData.isStop(stop),
      ...
      const int arrivalTime = sourceDepartureTime +
                              initialWalkingTransfers.getForwardDistance(stop);
    ...
    if (initialWalkingTransfers.getDistance() <= data.maxWalkTime) {
      ...
      const int arrivalTime =
          sourceDepartureTime + initialWalkingTransfers.getDistance();
  ...
  inline void relaxTransfers() noexcept {
    ...
    for (const StopId stop : stopsUpdatedByRoute) {
      ...
      if (initialWalkingTransfers.getBackwardDistance(stop) != INFTY) {
        ...
        const int arrivalTime =
            earliestArrivalTime +
            initialWalkingTransfers.getBackwardDistance(stop);
        if (arrivalByTransfer(targetStop, arrivalTime)) {
          EarliestArrivalLabel &label = currentRound()[targetStop];
          label.parent = stop;
          label.parentDepartureTime = earliestArrivalTime;
  ...
  inline void relaxRideTransfers() noexcept {
    for (const StopId fromStop : stopsUpdatedByRoute) {
      const int earliestArrivalTime =
          SeparateRouteAndTransferEntries
      ...
      for (const Edge edgeFrom : data.rideTransferGraph.edgesFrom(fromStop)) {
        const auto vehicleVertex =
            data.rideTransferGraph.get(ToVertex, edgeFrom);
        const auto vehId = data.getVehicleFromVertex(vehicleVertex);
        const auto pickupInfo =
            data.rideTransferGraph.get(InsertionInfo, edgeFrom);
        if (pickupInfo.vehicleId == RIDERAPTOR::noVehicleId) continue;

        profiler.countMetric(METRIC_TOTAL_DE,
                             data.rideTransferGraph.outDegree(vehicleVertex));

        if (optimizationFlags.UseTimePruning &&
            (pickupInfo.minDepTime >
                 earliestArrival[targetStop].getArrivalTimeByTransfer() ||
             pickupInfo.maxDepTime < earliestArrivalTime)) {
          profiler.countMetric(METRIC_FILTERED_PICKUP_EDGES);
          continue;
        }

        const auto pickupDetour =
            data.disp.getPickupDetour(earliestArrivalTime, pickupInfo);
        profiler.countMetric(METRIC_PICKUP_EDGES);

        for (const Edge edgeTo :
             data.rideTransferGraph.edgesFrom(vehicleVertex)) {
          const auto toStop =
              StopId(data.rideTransferGraph.get(ToVertex, edgeTo));
          if (toStop == sourceStop || toStop == fromStop) {
            continue;
          }
          const auto areStops = data.raptorData.isStop(fromStop) &&
                                data.raptorData.isStop(toStop);

          const auto dropoffInfo =
              data.rideTransferGraph.get(InsertionInfo, edgeTo);
          if (dropoffInfo.vehicleId == RIDERAPTOR::noVehicleId) continue;
          const auto latestArrival =
              std::min(earliestArrival[toStop].getArrivalTimeByTransfer(),
                       earliestArrival[targetStop].getArrivalTimeByTransfer());

          if (optimizationFlags.UseLeewayPruning && areStops &&
              pickupDetour > data.rideTransferGraph.get(Weight, edgeTo)) {
            profiler.countMetric(METRIC_LEEWAY_PRUNING);
            break;
          }

          if (optimizationFlags.UseTimePruning &&
              (dropoffInfo.minArrTime > latestArrival)) {
          ...
          const bool useDistance =
              optimizationFlags.UseDistanceMatrix && areStops;
          ...
          req.vehicleId = vehId;
          req.directDistance =
              useDistance ? data.distanceMatrix.getDistance(fromStop, toStop)
                          : INFTY;
          req.pickupInfo = pickupInfo;
          req.dropoffInfo = dropoffInfo;

          if (optimizationFlags.UseApproximation && !useDistance &&
              !data.disp.isFeasible(req, latestArrival)) {
            profiler.countMetric(METRIC_FILTERED_BY_APPROX);
            continue;
          }

          profiler.countMetric(METRIC_TRIED_RIDES);
          if (!useDistance) profiler.startExtraTimer();
          const auto [departureTime, arrivalTime] =
              useDistance ? data.disp.tryInsertionForVehicle<true>(req)
                          : data.disp.tryInsertionForVehicle<false>(req);
          if (!useDistance) profiler.countMetric(METRIC_CHSEARCHES);
          if (!useDistance) profiler.doneExtraTimer();
          if (arrivalTime != INFTY) profiler.countMetric(METRIC_FEASIBLE_RIDES);
