...
template <size_t GROUPED_ROUNDS, typename PROFILER = NoProfiler> class UPQuery {
...
public:
  ...
  using InitialAndFinalTransfers =
      RAPTOR::ParetoInitialAndFinalTransfers<false, GroupedRounds>;
...
private:
  struct QueryData {
    QueryData(const Data &oldData, const TransferGraph &oldTransferGraph,
        ...
        : transferGraph(oldTransferGraph), chData(oldCHData),
          targets(oldTargets) {
...
public:
  UPQuery(const Data &oldData, const TransferGraph &oldTransferGraph,
    ...
    profiler.registerPhases({PHASE_SCAN_INITIAL, PHASE_EVALUATE_INITIAL,
                             PHASE_SCAN_TRIPS, PHASE_FINAL_TRANSFERS});
...
public:
  ...
  inline std::vector<RAPTOR::Journey>
  getJourneys(const Vertex vertex) noexcept {
  ...
  inline std::vector<RAPTOR::ArrivalLabel>
  getArrivals(const Vertex vertex) noexcept {
    const Vertex internalVertex(queryData.externalToInternal[vertex]);
...
private:
    ...
    for (const RouteId route : data.routes()) {
      ...
      for (StopIndex stopIndex(0); stopIndex < endIndex; stopIndex++) {
        ...
        const int stopDepartureTime =
            initialAndFinalTransfers.getDistance(0, stop);
        if (stopDepartureTime == INFTY)
          continue;
    ...
    while (roundBegin < roundEnd) {
      ...
      for (size_t i = roundBegin; i < roundEnd; i++) {
        ...
        for (StopEventId j = label.begin; j < label.end; j++) {
          ...
          if (stopLabel.arrivalTime <= arrivalTime)
            continue;
          stopLabel.arrivalTime = arrivalTime;
          stopLabel.parent = i;
          initialAndFinalTransfers.template addSource<true>(stop, arrivalTime,
                                                            stop);
        }
      }
  ...
  inline void getJourney(std::vector<RAPTOR::Journey> &journeys, size_t round,
                         Vertex vertex) noexcept {
    const int arrivalTime =
        initialAndFinalTransfers.getTargetDistance(round, vertex);
    if (arrivalTime >=
        (journeys.empty() ? never : journeys.back().back().arrivalTime))
      return;
    RAPTOR::Journey result;
    if (round == 0) {
      result.emplace_back(Vertex(queryData.internalToExternal[sourceVertex]),
                          Vertex(queryData.internalToExternal[vertex]),
                          sourceDepartureTime, arrivalTime, false);
      journeys.emplace_back(result);
      return;
    }
    Vertex parentVertex =
        initialAndFinalTransfers.getTargetParent(round, vertex);
    ...
    while (parent != u_int32_t(-1)) {
      ...
      std::tie(arrivalStopEvent, edge) =
          (departureStopEvent == noStopEvent)
              ? getParent(label, parentVertex, stopLabel)
    ...
    const int initialTransferTime =
        initialAndFinalTransfers.getDistance(0, departureStop);
    result.emplace_back(sourceVertex, departureStop, sourceDepartureTime,
                        initialTransferTime, noEdge);
  ...
  inline std::pair<StopEventId, Edge>
  getParent(const TripLabel &parentLabel, const Vertex stop,
            const StopLabel &stopLabel) noexcept {
    ...
    for (StopEventId i = parentLabel.begin; i < end; i++) {
      if (data.arrivalEvents[i].stop != stop)
        continue;
      if (stopLabel.arrivalTime != data.arrivalEvents[i].arrivalTime)
        continue;
  ...
  inline void getArrival(std::vector<RAPTOR::ArrivalLabel> &labels,
                         size_t round, const Vertex vertex) noexcept {
    const int arrivalTime =
        initialAndFinalTransfers.getTargetDistance(round, vertex);
