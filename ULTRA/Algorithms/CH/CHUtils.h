...
inline std::vector<Vertex> getOrder(const CH &ch) noexcept {
  ...
  depthFirstSearch(
      ch, NoOperation, NoOperation,
      [&](const Edge edge, const Vertex) {
        order.push_back(ch.get(ToVertex, edge));
      },
      NoOperation, [&](const Vertex root) { order.push_back(root); });
  size_t front = 0;
...
vertexLevelTopDown(const CHGraph &ch,
  ...
  for (Vertex vertex : descending(order)) {
    for (Edge edge : ch.edgesFrom(vertex)) {
      level[vertex] =
          std::max<uint16_t>(level[vertex], level[ch.get(ToVertex, edge)] + 1);
    }
  }
  return level;
}
...
vertexLevelTopDown(const CH &ch, const std::vector<Vertex> &order) noexcept {
  ...
  for (Vertex vertex : descending(order)) {
    ...
    for (Edge edge : ch.backward.edgesFrom(vertex)) {
      level[vertex] = std::max<uint16_t>(
          level[vertex], level[ch.backward.get(ToVertex, edge)] + 1);
    }
  }
  return level;
}
...
vertexLevelBottomUp(const CH &ch, const std::vector<Vertex> &order) noexcept {
  ...
  for (Vertex vertex : order) {
    ...
    for (Edge edge : ch.backward.edgesFrom(vertex)) {
      level[ch.backward.get(ToVertex, edge)] = std::max<uint16_t>(
          level[vertex] + 1, level[ch.backward.get(ToVertex, edge)]);
    }
  }
  return level;
}
...
verticesByLevel(const CH &ch, const std::vector<uint16_t> &level) noexcept {
  ...
  for (Vertex vertex : ch.vertices()) {
    if (verticesByLevel.size() <= level[vertex])
      verticesByLevel.resize(level[vertex] + 1);
...
verticesPerLevel(const CH &ch, const std::vector<uint16_t> &level) noexcept {
  ...
  for (Vertex vertex : ch.vertices()) {
    if (verticesPerLevel.size() <= level[vertex])
      verticesPerLevel.resize(level[vertex] + 1, 0);
...
inline std::vector<Vertex> getLevelOrderBottomUp(const CH &ch) noexcept {
  return Vector::flatten(
      verticesByLevel(ch, vertexLevelBottomUp(ch, getOrder(ch))));
}
...
inline Data expandData(const CH &ch) noexcept {
  ...
  while (!data.order.empty() && ch.isCoreVertex(data.order.back())) {
    for (const Edge edge : ch.forward.edgesFrom(data.order.back())) {
      data.core.addEdge(data.order.back(), ch.forward.get(ToVertex, edge))
          .set(ViaVertex, ch.forward.get(ViaVertex, edge))
          .set(Weight, ch.forward.get(Weight, edge));
    }
    data.order.pop_back();
  }
  for (const Vertex vertex : data.order) {
    AssertMsg(!ch.isCoreVertex(vertex),
              "Vertex " << vertex << " is a core vertex!");
    for (const Edge edge : ch.forward.edgesFrom(vertex)) {
      data.forwardCH.addEdge(vertex, ch.forward.get(ToVertex, edge))
          .set(ViaVertex, ch.forward.get(ViaVertex, edge))
          .set(Weight, ch.forward.get(Weight, edge));
    }
    for (const Edge edge : ch.backward.edgesFrom(vertex)) {
      data.backwardCH.addEdge(vertex, ch.backward.get(ToVertex, edge))
          .set(ViaVertex, ch.backward.get(ViaVertex, edge))
          .set(Weight, ch.backward.get(Weight, edge));
    }
  }
  return data;
}
...
unpackShortcut(const GRAPH &forward, const GRAPH &backward, const Vertex from,
  ...
  while (!unprocessedShortCuts.empty()) {
    ...
    if (forward.isVertex(via)) {
      ...
      unprocessedShortCuts.push_back(
          Shortcut({forward.get(ViaVertex, second), to}));
      unprocessedShortCuts.push_back(
          Shortcut({backward.get(ViaVertex, first), via}));
    } else {
      path.push_back(to);
    }
  }
...
inline std::vector<Vertex> unpackShortcut(const CH &ch, const Vertex from,
                                          const Vertex via,
                                          const Vertex to) noexcept {
  return unpackShortcut(ch.forward, ch.backward, from, via, to);
}

inline void analyze(const CH &ch, const std::vector<Vertex> &order,
  ...
  out << "                  #Vertices : " << std::setw(tabSize)
      << String::prettyInt(vertexCount) << std::endl;
  ...
  out << "                     #Edges : " << std::setw(tabSize)
      << String::prettyInt(edgeCount) << std::endl;
  out << "                     #Level : " << std::setw(tabSize)
      << String::prettyInt(levelCount) << std::endl;
}
