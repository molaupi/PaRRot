...
class FullProfiler : public NoProfiler {
 ...
 private:
  ...
  inline void printStatistics() noexcept {
    ...
    const double missingPercent =
        missing / static_cast<double>(missing + contracted);
    const double coreDegree =
        data->core.numEdges() / static_cast<double>(missing);
    const double smooth = (expectedTime <= roundTime) ? 1 : 0.9;
    if (contracted > 0) {
      const double chDegree =
          (data->forwardCH.numEdges() + data->backwardCH.numEdges()) /
          static_cast<double>(contracted);
      const double contractionsPerMS = contractions / roundTime;
      const double shortcutsAddedPercent =
          shortcutsAdded / static_cast<double>(shortcutsTested);
      const double indeg = inDegSum / static_cast<double>(contractions);
      const double outdeg = outDegSum / static_cast<double>(contractions);
      const double witnessSearches =
          static_cast<double>(witnessSearchCount - lastWitnessSearchCount);
      const double witnessSearchPerContraction = witnessSearches / contractions;
      const double settledVerticesPerWitnessSearch =
          (settledVertices - lastSettledVertices) / witnessSearches;
      const double addedShortcutsPerWitnessSearch =
          shortcutsAdded / witnessSearches;
      expectedTime = (smooth * missing / contractionsPerMS) +
                     ((1 - smooth) * (expectedTime - roundTime));
      std::cout << std::setw(11) << String::prettyInt(contracted)
                ...
                << (String::prettyInt(shortcutsAdded) + "/" +
                    String::prettyInt(shortcutsTested))
