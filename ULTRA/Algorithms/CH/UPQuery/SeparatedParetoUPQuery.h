...
class SeparatedParetoUPQuery {
 ...
 public:
  ...
  SeparatedParetoUPQuery(const TransferGraph &transferGraph, const CH &ch,
                         const Order &&order,
                         const Vertex::ValueType numberOfStops,
                         const IndexedSet<false, Vertex> &targets,
                         const int direction = FORWARD)
      : SeparatedParetoUPQuery(transferGraph, ch.getGraph(direction),
  ...
  inline const TargetLabel &getTargetLabel(const size_t round,
                                           const Vertex vertex) const noexcept {
    const Vertex internalVertex = originalToInternal(vertex);
    return targetLabels[round][targetId[internalVertex]];
  }

  inline int getTargetDistance(const size_t round,
                               const Vertex vertex) const noexcept {
    return getTargetLabel(round, vertex).distance;
  }

  inline Vertex getTargetParent(const size_t round,
                                const Vertex vertex) const noexcept {
    return getTargetLabel(round, vertex).parent;
  }

  inline size_t numRounds() const noexcept { return targetLabels.size(); }
 ...
 private:
  ...
  inline void addSourceInternal(const Vertex vertex, const int initialDistance,
    ...
    if constexpr (FOR_SWEEP) {
      AssertMsg(upwardSweepGraph.externalToInternal(vertex) <
                    upwardSweepGraph.graph.numVertices(),
                "Vertex is not in sweep graph! (original: "
                    << internalToOriginal(vertex) << ", CH: " << vertex
                    << ", sweep: "
                    << upwardSweepGraph.externalToInternal(vertex) << ")");
      sweepStart =
          std::min(sweepStart, upwardSweepGraph.externalToInternal(vertex));
  ...
  inline void fullUpwardSweep() noexcept {
    if constexpr (Debug) {
      std::cout << "Running full upward sweep from " << sweepStart << "/"
  ...
  inline void restrictedUpwardSweep() noexcept {
    if constexpr (Debug) {
      std::cout << "Running restricted upward sweep from " << sweepStart << "/"
                << Vertex(reverseUpwardSweepGraph.graph.numVertices())
                << std::endl;
      timer.restart();
    }
    ...
    for (Vertex sweepV = sweepStart;
      ...
      for (const Edge edge : reverseUpwardSweepGraph.graph.edgesFrom(sweepV)) {
        ...
        timestamp[u] =
            branchlessConditional(update, currentTimestamp, timestamp[u]);
      }
    }
