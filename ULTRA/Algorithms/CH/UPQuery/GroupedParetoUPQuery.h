...
private:
  ...
  struct TargetLabel {
    TargetLabel(const int distance = never, const Vertex parent = noVertex)
        : distance(distance), parent(parent), timestamp(0) {}
    ...
    while (!Q.empty()) {
      ...
      for (Edge edge : searchGraph.edgesFrom(u)) {
        ...
        const int newDistance =
            distanceU->distance + searchGraph.get(TravelTime, edge);
        updateDistance(v);
    ...
    for (Vertex sweepV = sweepStart;
      ...
      for (const Edge edge : upwardSweepGraph.graph.edgesFrom(sweepV)) {
        ...
        for (size_t i = 1; i <= round; i++) {
          ...
          vLabel.distance[i] =
              branchlessConditional(update, newDistance, vLabel.distance[i]);
          vLabel.parent[i] =
              branchlessConditional(update, uLabel.parent[i], vLabel.parent[i]);
        }
      }
    }
  ...
  inline void downwardSearchToStops() noexcept {
    ...
    for (const Vertex sweepV : stopGraph.graph.vertices()) {
      ...
      for (const Edge edge : stopGraph.graph.edgesFrom(sweepV)) {
        ...
        vLabel.distance[0] =
            branchlessConditional(update, newDistance, vLabel.distance[0]);
        vLabel.parent[0] =
            branchlessConditional(update, uLabel.parent[0], vLabel.parent[0]);
      }
    }
...
private:
  ...
  inline void settleInitial() noexcept {
    ...
    if constexpr (StallOnDemand) {
      for (Edge edge : graph[BACKWARD].edgesFrom(u)) {
        ...
        if (distance[v].distance <
            distance[u].distance - graph[BACKWARD].get(Weight, edge))
          return;
      }
    }
    for (Edge edge : graph[FORWARD].edgesFrom(u)) {
      ...
      const int newDistance =
          distanceU->distance + graph[FORWARD].get(Weight, edge);
      updateDistanceInitial(v);
      if (distance[v].distance > newDistance) {
        distance[v].distance = newDistance;
        Q.update(&distance[v]);
