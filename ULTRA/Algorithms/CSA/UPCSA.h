...
public:
  ...
  using InitialAndFinalTransfers =
      RAPTOR::BasicInitialAndFinalTransfers<Debug, UseStopBuckets,
                                            UseTargetBuckets>;
...
private:
  ...
  struct QueryData {
    QueryData(const Data &oldData, const CH::CH &oldCHData,
        ...
        : data(oldData), chData(oldCHData), targets(oldTargets) {
  ...
  inline Journey getJourney(const Vertex vertex) noexcept {
    ...
    if (parent != internalVertex) {
      const int parentDepartureTime =
          (parent == sourceVertex) ? sourceDepartureTime : arrivalTime[parent];
      journey.emplace_back(parent, internalVertex, parentDepartureTime,
                           distance, noEdge);
    }
    ...
    while (stop != sourceVertex) {
      ...
      if (label.reachedByTransfer) {
        ...
        if (!journey.empty() && !journey.back().usesTrip) {
          ...
          journey.emplace_back(label.parent, stop, parentDepartureTime,
...
private:
  ...
  inline void runInitialTransfers() noexcept {
    ...
    for (const StopId stop : data.stops()) {
      arrivalByTransfer(stop, initialAndFinalTransfers.getDistance(stop),
                        sourceVertex, noEdge);
    }
  }
