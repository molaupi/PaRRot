...
public:
  ...
  OneToAllDijkstraCSA(const Data &data,
      ...
      : data(data), initialTransferGraph(initialTransferGraph),
        ...
        dijkstraParent(PathRetrieval ? initialTransferGraph.numVertices() : 0,
                       noVertex),
    ...
    profiler.registerPhases({PHASE_CLEAR, PHASE_INITIALIZATION,
                             PHASE_CONNECTION_SCAN, PHASE_FINAL_TRANSFERS});
  ...
  OneToAllDijkstraCSA(const Data &data,
                      const Profiler &profilerTemplate = Profiler())
      : OneToAllDijkstraCSA(data, data.transferGraph, TravelTime,
                            profilerTemplate) {}
  ...
  inline Journey getJourney(const Vertex vertex) noexcept {
    ...
    if (!data.isStop(vertex)) {
      ...
      journey.emplace_back(stop, vertex, arrivalTime[stop],
                           arrivalTime[vertex]);
    }
    while (stop != sourceVertex) {
      if (parentTrip[stop] == noTripId) {
        journey.emplace_back(parent[stop], stop, arrivalTime[parent[stop]],
                             arrivalTime[stop]);
      } else {
        journey.emplace_back(
            parent[stop], stop,
            data.connections[tripReached[parentTrip[stop]]].departureTime,
            arrivalTime[stop], parentTrip[stop]);
      }
...
private:
  ...
  inline void runDijkstra(const int nextDepartureTime) noexcept {
    while ((!queue.empty()) &&
           (queue.min().getArrivalTime() <= nextDepartureTime)) {
      settle(data.transferGraph, data.transferGraph[TravelTime]);
    }
  }

  template <typename GRAPH>
  inline void settle(const GRAPH &graph,
                     const std::vector<int> &weight) noexcept {
