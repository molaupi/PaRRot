...
struct RouteLabel {
  RouteLabel(const Data &data, const RouteId route)
    ...
    for (size_t i = 0; i < numberOfTrips; i++, trip++) {
      for (StopIndex stopIndex(0); stopIndex + 1 < numberOfStops; stopIndex++) {
        ...
        departureTimes[(stopIndex * numberOfTrips) + i] =
            data.departureTime(stopEvent);
      }
    }
  }
  ...
  inline bool findEarliestTripBinary(const StopIndex stopIndex,
    ...
    tripIndex =
        std::lower_bound(TripId(0), TripId(numberOfTrips), departureTime,
  ...
  inline bool findEarliestTripLinear(const StopIndex stopIndex,
    ...
    tripIndex--;
    while ((tripIndex > 0) &&
           (departureTimes[labelIndex + tripIndex - 1] >= departureTime)) {
      tripIndex--;
    }
    return true;
  }
