...
#include "../../Algorithms/Loud/BaseObjects/Request.h"
#include "../../Algorithms/Loud/BaseObjects/Vehicle.h"
...
using CHProvider =
    Loud::StandardCHProvider<RoadGraph, ImplementationDetail::TravelTimeType>;
 ...
 public:
  ...
  void buildRideTransferGraph() {
    ...
    for (int vehId = 0; vehId < accumulatedNumStops.size() - 1; vehId++) {
      const int numStops =
          accumulatedNumStops[vehId + 1] - accumulatedNumStops[vehId];
  ...
  void extendRideTransferGraph(StopId sourceStop, Edge sourceEdge,
    ...
    if (!raptorData.isStop(sourceStop)) {
      ...
      const auto insertionInfos =
          disp.runBCHSearchFromStop(sourceTail, sourceHead, travelTime);
    ...
    if (!raptorData.isStop(targetStop)) {
      ...
      const auto insertionInfos =
          disp.runBCHSearchFromStop(targetTail, targetHead, travelTime);
 ...
 private:
  ...
  void insertVehicleEdges(const std::vector<StopInsertionInfo> insertionInfos,
    ...
    for (const auto insertionInfo : insertionInfos) {
      auto edge =
          constructionGraph
              .addEdge(Vertex(stop), getVertexFromVehiclePosition(
                                         insertionInfo.vehicleId,
      ...
      if (INSERT_OUTGOING) {
        ...
        const auto &capacity =
            disp.fleet[insertionInfo.vehicleId].seatingCapacity;

        for (int pos = insertionInfo.insertionPosition; pos >= 0; pos--) {
          if (occupancies[pos] == capacity) break;
          auto reverseEdge =
              constructionGraph
                  .addEdge(getVertexFromVehiclePosition(insertionInfo.vehicleId,
                                                        pos),
                           Vertex(stop))
                  .set(Weight, (pos == insertionInfo.insertionPosition
                                    ? insertionInfo.detour
                                    : 0) +
      ...
      stopCounter[accumulatedNumStops[insertionInfo.vehicleId] +
                  insertionInfo.insertionPosition]++;
    }
  }
  ...
  void insertTargetEdges(const std::vector<StopInsertionInfo> insertionInfos,
    ...
    for (auto vertex = numberOfStops; vertex < rideTransferGraph.numVertices();
         vertex++) {
      const auto edge =
          rideTransferGraph.findEdge(Vertex(vertex), targetVertex);
      rideTransferGraph.set(Weight, edge, INFTY);
      StopInsertionInfo info;
      rideTransferGraph.set(InsertionInfo, edge, info);
    }
