...
struct TimeData {
  ...
  inline void printValues() const noexcept {
    double timePerBCH =
        phaseTime[PHASE_BCHSEARCHES] / metricValue[METRIC_BCHSEARCHES];
    double stationsPerVehicle =
        double(metricValue[METRIC_NEARBYVEHICLEROUTES]) /
        double(metricValue[METRIC_VEHICLES]);
    double stationsPerVehicleStop =
        double(metricValue[METRIC_NEARBYVEHICLEROUTES]) /
        double(metricValue[METRIC_VEHICLESTOPS] - metricValue[METRIC_VEHICLES]);
    double vehiclesPerStation =
        double(metricValue[METRIC_NEARBYVEHICLEROUTES]) /
        double(metricValue[METRIC_STATIONS]);

    double pickupEdgesPerStation = double(metricValue[METRIC_PICKUPEDGES]) /
                                   double(metricValue[METRIC_STATIONS]);
    double dropoffEdgesPerStation = double(metricValue[METRIC_DROPOFFEDGES]) /
                                    double(metricValue[METRIC_STATIONS]);
    double pickupEdgesPerVehicleStop = double(metricValue[METRIC_PICKUPEDGES]) /
                                       double(metricValue[METRIC_VEHICLESTOPS]);
    double dropoffEdgesPerVehicleStop =
        double(metricValue[METRIC_DROPOFFEDGES]) /
        double(metricValue[METRIC_VEHICLESTOPS]);
  ...
  inline void writeValuesToFile(
    ...
    additionalValues.push_back(double(metricValue[METRIC_NEARBYVEHICLEROUTES]) /
                               double(metricValue[METRIC_VEHICLESTOPS] -
                                      metricValue[METRIC_VEHICLES]));
    additionalValues.push_back(double(metricValue[METRIC_NEARBYVEHICLEROUTES]) /
                               double(metricValue[METRIC_STATIONS]));
...
class Profiler : public NoProfiler {
 public:
  Profiler() : totalTime(0.0), timer(NUM_PHASES) {}
