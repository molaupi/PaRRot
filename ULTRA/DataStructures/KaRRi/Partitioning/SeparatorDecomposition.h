...
// A separator decomposition of an undirected n-vertex graph G = (V, E) is a
// rooted tree T whose nodes are disjoint subsets of V and that is recursively
// defined as follows. If n = 1, then T consists of a single node equal to V. If
// n > 1, then T consists of a root whose removal separates G into several
// subgraphs. The children of the root are the roots of separator decompositions
// of these subgraphs.
struct SeparatorDecomposition {
  ...
  // Returns the index in the order of the first separator vertex contained in
  // this node.
  ...
  // Returns the index in the order one past the last separator vertex contained
  // in this node.
