...
public:
  ...
  inline double distanceToLine(const Point &p, const Point &q) const {
    ...
    const Point closestPointOnLine = p + (dotProduct(*this - p, direction) *
                                          (direction / direction.absSquared()));
...
inline double geoDistanceInCM(const Point &from, const Point &to) {
  ...
  return acos(std::min(1.0, sin(widthFrom) * sin(widthTo) +
                                cos(widthFrom) * cos(widthTo) *
                                    cos(heightTo - heightFrom))) *
