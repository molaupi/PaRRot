...
template <typename ATTRIBUTES, typename INDEX = size_t> class AttributeHandle {
...
public:
  ...
  using AttributeType =
      typename Attributes::template AttributeType<ATTRIBUTE_NAME>;

  template <AttributeNameType ATTRIBUTE_NAME>
  using AttributeReferenceType =
      typename Attributes::template AttributeReferenceType<ATTRIBUTE_NAME>;

  template <AttributeNameType ATTRIBUTE_NAME>
  using AttributeConstReferenceType =
      typename Attributes::template AttributeConstReferenceType<ATTRIBUTE_NAME>;
...
public:
  ...
  inline Type &set(const AttributeNameWrapper<ATTRIBUTE_NAME> attributeName,
                   const TYPE &value) noexcept {
    attributes.set(attributeName, index, value);
    return *this;
  }
  template <typename RECORD> inline Type &set(const RECORD &record) noexcept {
