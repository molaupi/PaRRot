...
class DynamicGraphImplementation {
    static_assert(
        ...
        "Edge!");
    static_assert(
        Meta::Equals<size_t, Meta::FindAttributeType<OutDegree, LIST_OF_VERTEX_ATTRIBUTES>>(),
    ...
    static_assert(
        ...
        "A dynamic graph requires a vertex attribute named IncomingEdges of type "
        "vector<Edge>!");
    static_assert(
        Meta::Equals<bool,
    ...
    static_assert(
        Meta::Equals<size_t, Meta::FindAttributeType<IncomingEdgePointer, LIST_OF_EDGE_ATTRIBUTES>>(),
        "A dynamic graph requires an edge attribute named IncomingEdgePointer of "
        "type size_t!");
    static_assert(
        Meta::Equals<Vertex, Meta::FindAttributeType<FromVertex, LIST_OF_EDGE_ATTRIBUTES>>(),
...
public:
    ...
    {
        changeVertexIds(ULTRAPermutation(Construct::Invert, order));
    }
    ...
    inline void packEdges() noexcept
    {
        ...
        size_t i = 0;
        ULTRAPermutation permutation(edgeAttributes.size());
        for (const Vertex vertex : vertices()) {
    ...
    inline void revert() noexcept
    {
        ...
        const ULTRAPermutation edgePermutation(Construct::Invert,
...
private:
    inline void moveEdge(const Edge oldEdge, const Edge newEdge) noexcept
    {
        ...
        get(IncomingEdges,
            get(ToVertex, newEdge))[get(IncomingEdgePointer, newEdge)]
            = newEdge;
        if constexpr (HasEdgeAttribute(ReverseEdge)) {
        ...
        const Edge edge) noexcept
    {
        ...
        if (get(IncomingEdgePointer, edge) != get(IncomingEdges, to).size() - 1) {
            AssertMsg(
                get(IncomingEdgePointer, get(IncomingEdges, to).back()) == get(IncomingEdges, to).size() - 1,
    ...
    inline void changeVertexIds(const ULTRAPermutation& permutation) noexcept
    {
