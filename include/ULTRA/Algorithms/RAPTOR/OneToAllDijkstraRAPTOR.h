...
public:
    ...
    OneToAllDijkstraRAPTOR(const Data& data, const TransferGraph& forwardGraph,
        const TransferGraph& backwardGraph,
        const Profiler& profilerTemplate = Profiler())
        : OneToAllDijkstraRAPTOR(data, forwardGraph, backwardGraph, TravelTime,
            profilerTemplate)
    {
    }
        ...
        const size_t round) const noexcept
    {
        if (data.isStop(vertex)) {
            ...
            AssertMsg(dijkstraLabels[round][vertex].arrivalTime < never,
                "No label found for vertex " << vertex << " in round " << round
                                             << "!");
...
private:
        ...
        for (const StopId stop : stopsUpdatedByTransfer) {
            for (const RouteSegment& route : data.routesContainingStop(stop)) {
                ...
                if (route.stopIndex + 1 == data.numberOfStopsInRoute(route.routeId))
                    continue;
                if (data.lastTripOfRoute(route.routeId)[route.stopIndex].departureTime < previousRound()[stop].arrivalTime)
                    continue;
    ...
    inline void relaxInitialTransfers() noexcept
    {
        ...
        for (const Vertex vertex : initialTransfers.getForwardPOIs()) {
            AssertMsg(initialTransfers.getForwardDistance(vertex) != INFTY,
                "Vertex " << vertex << " was not reached!");
            ...
            if (data.isStop(vertex)) {
                arrivalByTransfer(StopId(vertex), label.arrivalTime, sourceVertex,
                    sourceDepartureTime);
            }
        }
    }
    ...
    inline void dijkstra() noexcept
    {
        while (!queue.empty()) {
            ...
            for (Edge edge : data.transferGraph.edgesFrom(u)) {
                ...
                arrivalByEdge(
                    v, uLabel->arrivalTime + data.transferGraph.get(TravelTime, edge),
                    uLabel->parent);
            }
            if (data.isStop(u)) {
                arrivalByTransfer(StopId(u), uLabel->arrivalTime, uLabel->parent,
                    earliestArrival[uLabel->parent]);
            }
    ...
    inline bool arrivalByEdge(const Vertex vertex, const int arrivalTime,
        const Vertex parent) noexcept
    {
        AssertMsg(data.isStop(parent),
            "Parent vertex (" << parent << ") is not a stop");
        AssertMsg(arrivalTime >= sourceDepartureTime,
            "Arriving by route BEFORE departing from the source (source "
            "departure time: "
                << String::secToTime(sourceDepartureTime) << " ["
        ...
        Vertex vertex) const noexcept
    {
        Journey journey;
        if (data.isStop(vertex)) {
            if (rounds[round][vertex].arrivalTime >= (journeys.empty() ? never : journeys.back().back().arrivalTime))
                return;
        } else {
            const DijkstraLabel& label = dijkstraLabels[round][vertex];
            if (label.arrivalTime >= (journeys.empty() ? never : journeys.back().back().arrivalTime))
                return;
            AssertMsg(label.parent != noVertex,
                "Vertex " << vertex << ": Label with " << round
                          << " trips has no parent! (arrival time: "
                          << label.arrivalTime << ")");
            journey.emplace_back(label.parent, vertex,
                rounds[round][label.parent].arrivalTime,
                label.arrivalTime, false, noRouteId);
            vertex = label.parent;
        }
    ...
    inline void getArrival(std::vector<ArrivalLabel>& labels, const size_t round,
        const Vertex vertex) const noexcept
    {
    ...
    inline void printRoundsForStop(const StopId stop) const noexcept
    {
        ...
        std::cout << std::setw(10) << "Round" << std::setw(14) << "arrivalTime"
                  << std::setw(14) << "parent" << std::endl;
        for (size_t i = 0; i < rounds.size(); i++) {
            std::cout << std::setw(10) << i << std::setw(14)
                      << rounds[i][stop].arrivalTime << std::setw(14)
