...
private:
    ...
    struct QueryData {
        QueryData(const Data& oldData, const ULTRACH::CH& oldCHData, const IndexedSet<false, Vertex>& oldTargets, const bool reorder, const bool useDFSOrder) :
            data(oldData),
            chData(oldCHData),
            targets(oldTargets) {
...
public:
    UPCSA(const Data& oldData, const ULTRACH::CH& oldCHData, const IndexedSet<false, Vertex>& oldTargets, const bool reorder, const bool useDFSOrder, const Profiler& profilerTemplate = Profiler()) :
        ...
        Assert(!ULTRAGraph::hasLoops(data.transferGraph), "Shortcut graph may not have loops!");
