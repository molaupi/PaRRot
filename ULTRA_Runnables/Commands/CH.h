...
template<typename PROFILER>
using BidirectionalWitnessSearch = ULTRACH::BidirectionalWitnessSearch<CHCoreGraph, PROFILER, BidirectionalPopLimit>;
...
inline ULTRACH::CH finalizeCH(CH_BUILDER&& chBuilder, const std::string& orderOutputFile, const std::string& chOutputFile) noexcept {
...
inline ULTRACH::CH buildCH(GRAPH& originalGraph, const std::string& orderOutputFile, const std::string& chOutputFile, const KEY_FUNCTION& keyFunction, const STOP_CRITERION& stopCriterion = StopCriterion()) noexcept {
    TravelTimeGraph graph;
    ULTRAGraph::copy(originalGraph, graph);
    ULTRAGraph::printInfo(graph);
    ULTRACH::Builder<PROFILER, WITNESS_SEARCH, KEY_FUNCTION, STOP_CRITERION, false, false> chBuilder(std::move(graph), graph[TravelTime], keyFunction, stopCriterion);
    chBuilder.run();
...
private:
    ...
    inline void build() const noexcept {
        ...
        const ULTRACH::CH ch = buildCH<PROFILER, WITNESS_SEARCH>(data.transferGraph, getParameter("Order output file"), getParameter("CH output file"), keyFunction, stopCriterion);
