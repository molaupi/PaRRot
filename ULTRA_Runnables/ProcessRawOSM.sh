...
${cfg[osmium_executable]} extract -p "${cfg[boundary_inner]}" -o "${osm_dir}"/"${cfg[instance_name]}"_tmp_FullInner.osm.pbf "${osm_dir}"/tmp_osm_source_Highways.osm.pbf
...
rm "${osm_dir}"/tmp_osm_source_Highways.osm.pbf "${osm_dir}"/tmp_osm_source_VehicleHighways.osm.pbf
