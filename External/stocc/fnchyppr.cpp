...
double CFishersNCHypergeometric::probability(int32_t x)
{
    ...
    if (odds == 1.) {
        ...
        return exp(
            LnFac(m) - LnFac(x) - LnFac(m - x) + LnFac(N - m) - LnFac(n - x) - LnFac((N - m) - (n - x)) - (LnFac(N) - LnFac(n) - LnFac(N - n)));
    }

    if (odds == 0.) {
        if (n > N - m)
            FatalError("Not enough items with nonzero weight in CFishersNCHypergeometric::probability");
        return x == 0;
    }
...
double CFishersNCHypergeometric::probabilityRatio(int32_t x, int32_t x0)
{
    ...
    if (dx <= 28 && x <= 100000) { // avoid overflow
        ...
        // use logarithms
        f1 = FallingFactorial(a1, dx) + FallingFactorial(a2, dx) - FallingFactorial(a3, dx) - FallingFactorial(a4, dx) + dx * log(odds);
        if (invert)
            f1 = -f1;
        f1 = exp(f1);
    }
...
CMultiFishersNCHypergeometric::CMultiFishersNCHypergeometric(int32_t n_, int32_t* m_, double* odds_, int colors_, double accuracy_)
{
    ...
    for (N = N1 = 0, i = 0; i < colors; i++) {
        if (m[i] < 0 || odds[i] < 0)
            FatalError("Parameter negative in constructor for CMultiFishersNCHypergeometric");
        N += m[i];
        if (odds[i])
            N1 += m[i];
    }
    if (N < n)
        FatalError("Not enough items in constructor for CMultiFishersNCHypergeometric");
    if (N1 < n)
        FatalError("Not enough items with nonzero weight in constructor for CMultiFishersNCHypergeometric");
...
void CMultiFishersNCHypergeometric::mean(double* mu)
{
    ...
    do {
        ...
        r *= n * (N - q) / (q * (N - n));
        if (++iter > 100)
            FatalError("convergence problem in function CMultiFishersNCHypergeometric::mean");
