...
def merge_files(karri_requests_path, ultra_requests_path, output_path):
    """
    Merges two CSV files line-by-line using pandas.
    Takes origin, destination and req_time from karri_requests and source, target from ultra_requests.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ultra_requests', help="Path to the ULTRA requests CSV file (with source, target).")
    ...
    if args.mode == 'modify_stops':
        ...
        if not all([args.karri_requests, args.ultra_requests, args.output]):
            parser.error("--karri_requests, --ultra_requests, and --output are required for 'merge_files' mode.")
        merge_files(args.karri_requests, args.ultra_requests, args.output)