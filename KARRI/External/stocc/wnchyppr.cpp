...
double pow2_1(double q, double *y0 = 0) {
  ...
  if (y0)
    *y0 = y; // Return y if not void pointer
  return y1; // Return y1
}
...
double LnFacr(double x) {
  ...
  if (x == ix)
    return LnFac(ix); // x is integer
  double r, r2, D = 1., f;
  static const double C0 = 0.918938533204672722, // ln(sqrt(2*pi))
      C1 = 1. / 12., C3 = -1. / 360., C5 = 1. / 1260., C7 = -1. / 1680.;
  if (x < 6.) {
    if (x == 0 || x == 1)
      return 0;
    while (x < 6)
...
#if defined(_M_IX86) || defined(__INTEL__) || defined(_M_X64) ||               \
...
CWalleniusNCHypergeometric::CWalleniusNCHypergeometric(int32_t n_, int32_t m_,
                                                       int32_t N_, double odds_,
                                                       double accuracy_) {
  // constructor
  accuracy = accuracy_;
...
void CWalleniusNCHypergeometric::SetParameters(int32_t n_, int32_t m_,
                                               int32_t N_, double odds) {
  // change parameters
  if (n_ < 0 || n_ > N_ || m_ < 0 || m_ > N_ || odds < 0)
    FatalError("Parameter out of range in CWalleniusNCHypergeometric");
  n = n_;
  m = m_;
  N = N_;
  omega = odds; // set parameters
  xmin = m + n - N;
  if (xmin < 0)
    xmin = 0; // calculate xmin
  xmax = n;
  if (xmax > m)
    xmax = m;                      // calculate xmax
...
double CWalleniusNCHypergeometric::mean(void) {
  ...
  if (omega == 0.) {
    if (n > N - m)
      FatalError("Not enough items with nonzero weight in "
                 "CWalleniusNCHypergeometric::mean");
    return 0.;
  }
  ...
  // calculate Cornfield mean of Fisher noncentral hypergeometric distribution
  // as first guess
  a = (m + n) * omega + (N - m - n);
  ...
  if (omega > 1.) {
    do { // Newton Raphson iteration
      ...
      if (mean > xmax)
        mean = xmax;
      if (++iter > 40) {
        FatalError("Search for mean failed in function "
                   "CWalleniusNCHypergeometric::mean");
      }
...
double CWalleniusNCHypergeometric::variance(void) {
  ...
  // find approximate variance from Fisher's noncentral hypergeometric
  // approximation
  double r1 = my * (m - my);
  double r2 = (n - my) * (my + N - n - m);
  if (r1 <= 0. || r2 <= 0.)
    return 0.;
...
void CWalleniusNCHypergeometric::findpars() {
  ...
  do {
    ...
    if (zd == 0)
      FatalError(
          "can't find r in function CWalleniusNCHypergeometric::findpars");
    rr -= z / zd;
    if (rr <= d1)
      rr = lastr * 0.125 + d1 * 0.875;
    if (++j == 70)
      FatalError("convergence problem searching for r in function "
                 "CWalleniusNCHypergeometric::findpars");
  ...
  if (phi2d >= 0.) {
    FatalError("peak width undefined in function "
               "CWalleniusNCHypergeometric::findpars");
    /* wr = r = 0.; */
  } else {
    wr = sqrt(-phi2d);
...
int CWalleniusNCHypergeometric::BernouilliH(int32_t x_, double h, double rh,
                                            StochasticLib1 *sto) {
  ...
  if (G_integral <= h) { // G fits under h-hat
    do {
      ts = sto->Normal(0, k); // sample ts from normal distribution
             ...
             0.5); // reject values outside interval, and avoid ts = 0
    ...
    for (fts = 0., j = 0; j < 2; j++) { // calculate (Phi(ts)+Phi(1-ts))/2
      ...
      fts += exp(log1pow(rlogts * omega, xi[0]) + log1pow(rlogts, xi[1]) +
                 rdm1 * logts + bico);
      ts = 1. - ts;
    }
    fts *= 0.5;
...
double CWalleniusNCHypergeometric::recursive() {
  ...
  // Approximate by ignoring probabilities < accuracy and minimize storage
  // requirement
  ...
  for (nu = 1; nu <= n; nu++) {
    if (n - nu < x - x1 || p1[x1] < accuracya) {
      x1++; // increase lower limit when breakpoint passed or probability
            // negligible
    ...
    if (p2 + x2 - p > BUFSIZE)
      FatalError(
          "buffer overrun in function CWalleniusNCHypergeometric::recursive");
...
double CWalleniusNCHypergeometric::binoexpand() {
  ...
  // only for x < 2 or n-x < 2 (not implemented for higher x because of loss of
  // precision)
  int32_t x1, m1, m2;
  double o;
  if (x > n / 2) { // invert
  ...
  FatalError(
      "x > 1 not supported by function CWalleniusNCHypergeometric::binoexpand");
  return 0;
}
...
double CWalleniusNCHypergeometric::laplace() {
  ...
  PrecisionIndex =
      (-FloorLog2((float)accuracy) - ERFRES_B + ERFRES_S - 1) / ERFRES_S;
  if (PrecisionIndex < 0)
    PrecisionIndex = 0;
  ...
  while (w * NumSDev[PrecisionIndex] > 0.3) {
    ...
    if (PrecisionIndex == 0) {
      FatalError("Laplace method failed. Peak width too high in function "
                 "CWalleniusNCHypergeometric::laplace");
      break;
    }
  ...
  for (k = 3; k <= degree; k++) {
    ...
    // The j'th derivatives of psi are identical to the derivatives of phi for
    // j>2, and zero for j=1,2. Hence we are using phideri[j] for j>2 here.
...
double CWalleniusNCHypergeometric::integrate() {
  ...
  if (w < 0.02 || (w < 0.1 && (x == m || n - x == N - m) && accuracy > 1E-6)) {
    ...
    ta = 0.5 + 0.5 * delta;
    sum = integrate_step(1. - ta,
                         ta); // first integration step around center peak
    do {
      tb = ta + delta;
      if (tb > 1.)
        tb = 1.;
      s = integrate_step(ta, tb); // integration step to the right of peak
      s += integrate_step(1. - tb,
                          1. - ta); // integration step to the left of peak
      sum += s;
      if (s < accuracy * sum)
        break; // stop before interval finished if accuracy reached
      ta = tb;
      if (tb > 0.5 + w)
        delta *= 2.; // increase step length far from peak
    } while (tb < 1.);
  } else {
    ...
    for (t1 = 0., t2 = 0.5; t1 < 1.; t1 += 0.5, t2 += 0.5) {
      ...
      do {
        ...
        if (s < sum * 1E-4)
          delta1 *= 8.; // large step when s small
        ta = tb;
      } while (tb < t2);
      if (tinf) {
        ...
        do {
          ...
          if (s < sum * 1E-4)
            delta *= 8.; // large step when s small
          tb = ta;
        } while (ta > t1);
      }
    }
...
#define IPOINTS 8 // number of points in each integration step
  ...
  static const double weights[3] = {.5555555555555555, .88888888888888888,
                                    .55555555555555};
#elif IPOINTS == 4
  static const double xval[4] = {-0.861136311594, -0.339981043585,
                                 0.339981043585, 0.861136311594},
                      static const double weights[4] = {
                          0.347854845137, 0.652145154863, 0.652145154863,
                          0.347854845137};
#elif IPOINTS == 5
  static const double xval[5] = {-0.906179845939, -0.538469310106, 0,
                                 0.538469310106, 0.906179845939};
  static const double weights[5] = {0.236926885056, 0.478628670499,
                                    0.568888888889, 0.478628670499,
                                    0.236926885056};
#elif IPOINTS == 6
  static const double xval[6] = {-0.932469514203, -0.661209386466,
                                 -0.238619186083, 0.238619186083,
                                 0.661209386466,  0.932469514203};
  static const double weights[6] = {0.171324492379, 0.360761573048,
                                    0.467913934573, 0.467913934573,
                                    0.360761573048, 0.171324492379};
#elif IPOINTS == 8
  static const double xval[8] = {
      -0.960289856498, -0.796666477414, -0.525532409916, -0.183434642496,
      0.183434642496,  0.525532409916,  0.796666477414,  0.960289856498};
  static const double weights[8] = {
      0.10122853629,  0.222381034453, 0.313706645878, 0.362683783378,
      0.362683783378, 0.313706645878, 0.222381034453, 0.10122853629};
#elif IPOINTS == 12
  static const double xval[12] = {
      -0.981560634247, -0.90411725637,  -0.769902674194, -0.587317954287,
      -0.367831498998, -0.125233408511, 0.125233408511,  0.367831498998,
      0.587317954287,  0.769902674194,  0.90411725637,   0.981560634247};
  static const double weights[12] = {
      0.0471753363866, 0.106939325995, 0.160078328543, 0.203167426723,
      0.233492536538,  0.249147045813, 0.249147045813, 0.233492536538,
      0.203167426723,  0.160078328543, 0.106939325995, 0.0471753363866};
#elif IPOINTS == 16
  static const double xval[16] = {
      -0.989400934992, -0.944575023073, -0.865631202388, -0.755404408355,
      -0.617876244403, -0.458016777657, -0.281603550779, -0.0950125098376,
      0.0950125098376, 0.281603550779,  0.458016777657,  0.617876244403,
      0.755404408355,  0.865631202388,  0.944575023073,  0.989400934992};
  static const double weights[16] = {
      0.027152459411, 0.0622535239372, 0.0951585116838, 0.124628971256,
      0.149595988817, 0.169156519395,  0.182603415045,  0.189450610455,
      0.189450610455, 0.182603415045,  0.169156519395,  0.149595988817,
      0.124628971256, 0.0951585116838, 0.0622535239372, 0.027152459411};
#else
...
double CWalleniusNCHypergeometric::search_inflect(double t_from, double t_to) {
  ...
  do {
    ...
    for (i = 0; i < COLORS; i++) { // calculate first 3 derivatives of phi(t)
      ...
      phi[3] -=
          xx[i] * q * (zeta[i][1][3] + q * (zeta[i][2][3] + q * zeta[i][3][3]));
    }
    phi[1] += rdm1;
    phi[2] -= rdm1;
    phi[3] += 2. * rdm1;
    ...
    Zd = method * phi[1] * phi[1] * phi[1] + (2. + method) * phi[1] * phi[2] +
         phi[3];
    ...
    if (t <= t_from)
      t = (t1 + t_from) * 0.5;
    if (++iter > 20)
      FatalError("Search for inflection point failed in function "
                 "CWalleniusNCHypergeometric::search_inflect");
...
double CWalleniusNCHypergeometric::probability(int32_t x_) {
  ...
  if (omega == 0.) {
    if (n > N - m)
      FatalError("Not enough items with nonzero weight in "
                 "CWalleniusNCHypergeometric::probability");
    return x == 0;
  }
...
int32_t CWalleniusNCHypergeometric::MakeTable(double *table, int32_t MaxLength,
                                              int32_t *xfirst, int32_t *xlast,
                                              double cutoff) {
  ...
  if (MaxLength <= 0) {
    ...
    if (!UseTable && i1 > 200) {
      ...
      // estimate number of standard deviations to include from normal
      // distribution
  ...
  if (UseTable && MaxLength > LengthNeeded) {
    ...
    for (nu = 1; nu <= n; nu++) {
      ...
      for (xi = x2; xi >= x1; xi--) { // backwards loop
        ...
        y = p1[xi - 1] * mxo * d2 * dcom + y1 * (Nmnx + 1) * d1 * dcom;
        y1 = p1[xi -
                1]; // (warning: pointer alias, can't swap instruction order)
        p2[xi] = y;
      }
      p1 = p2;
    }
    ...
    *xlast = x1 + i2 - 1;
    if (i2 > 0)
      memmove(table, table + 1,
              i2 * sizeof(table[0])); // copy to start of table
...
CMultiWalleniusNCHypergeometric::CMultiWalleniusNCHypergeometric(
    int32_t n_, int32_t *m_, double *odds_, int colors_, double accuracy_) {
  // constructor
  accuracy = accuracy_;
  SetParameters(n_, m_, odds_, colors_);
}

void CMultiWalleniusNCHypergeometric::SetParameters(int32_t n_, int32_t *m_,
                                                    double *odds_,
                                                    int colors_) {
  // change parameters
  int32_t N1;
  ...
  for (N = N1 = 0, i = 0; i < colors; i++) {
    if (m[i] < 0 || omega[i] < 0)
      FatalError("Parameter negative in constructor for "
                 "CMultiWalleniusNCHypergeometric");
    N += m[i];
    if (omega[i])
      N1 += m[i];
  }
  if (N < n)
    FatalError(
        "Not enough items in constructor for CMultiWalleniusNCHypergeometric");
  if (N1 < n)
    FatalError("Not enough items with nonzero weight in constructor for "
               "CMultiWalleniusNCHypergeometric");
}

void CMultiWalleniusNCHypergeometric::mean(double *mu) {
  // calculate approximate mean of multivariate Wallenius noncentral
...
double CMultiWalleniusNCHypergeometric::binoexpand(void) {
  ...
  for (i = j = k = 0; i < colors; i++) {
    ...
    if (x[i]) {
      j = i;
      k++; // find the nonzero x[i]
    }
  }
  if (k > 1)
    FatalError("More than one x[i] nonzero in "
               "CMultiWalleniusNCHypergeometric::binoexpand");
...
void CMultiWalleniusNCHypergeometric::findpars(void) {
  ...
  do {
    ...
    if (zd == 0)
      FatalError(
          "can't find r in function CMultiWalleniusNCHypergeometric::findpars");
    rr -= z / zd; // next r
    if (rr <= dr)
      rr = lastr * 0.125 + dr * 0.875;
    if (++j == 70)
      FatalError("convergence problem searching for r in function "
                 "CMultiWalleniusNCHypergeometric::findpars");
...
double CMultiWalleniusNCHypergeometric::search_inflect(double t_from,
                                                       double t_to) {
  // search for an inflection point of the integrand PHI(t) in the interval
  // t_from < t < t_to
  double t, t1;                 // independent variable
  ...
  do {
    ...
    for (i = 0; i < colors; i++) { // calculate first 3 derivatives of phi(t)
      ...
      phi[3] -=
          x[i] * q * (zeta[i][1][3] + q * (zeta[i][2][3] + q * zeta[i][3][3]));
    }
    phi[1] += rdm1;
    phi[2] -= rdm1;
    phi[3] += 2. * rdm1;
    ...
    if (++iter > 20)
      FatalError("Search for inflection point failed in function "
                 "CMultiWalleniusNCHypergeometric::search_inflect");
  } while (fabs(t - t1) > 1E-5);
  return t;
}
...
double CMultiWalleniusNCHypergeometric::probability(int32_t *x_) {
  ...
  for (xsum = i = 0; i < colors; i++)
    xsum += x[i];
  if (xsum != n) {
    FatalError("sum of x values not equal to n in function "
               "CMultiWalleniusNCHypergeometric::probability");
  }
  ...
  if (colors < 3) {
    ...
    if (omega[1] == 0.)
      return x[0] == m[0];
    return CWalleniusNCHypergeometric(n, m[0], N, omega[0] / omega[1], accuracy)
        .probability(x[0]);
  }

  central = 1;
  for (i = j = em = 0; i < colors; i++) {
    if (x[i] > m[i] || x[i] < 0 || x[i] < n - N + m[i])
      return 0.;
    if (x[i] > 0)
      j++;
    if (omega[i] == 0. && x[i])
...
double CMultiWalleniusNCHypergeometricMoments::moments(double *mu,
                                                       double *variance,
