...
// #include "wnchyppr.cpp"              // calculate Wallenius noncentral
// hypergeometric probability #include "fnchyppr.cpp"              // calculate
// Fisher's noncentral hypergeometric probability
...
int32_t StochasticLib3::WalleniusNCHyp(int32_t n, int32_t m, int32_t N,
                                       double odds) {
  /*
  ...
  */
  ...
  if (n >= N || m >= N || n <= 0 || m <= 0 || odds <= 0.) {
    ...
    if (odds == 0.) {
      if (n > N - m)
        FatalError(
            "Not enough items with nonzero weight in function WalleniusNCHyp");
      return 0;
    }
    // illegal parameter
...
int32_t StochasticLib3::WalleniusNCHypUrn(int32_t n, int32_t m, int32_t N,
                                          double odds) {
...
int32_t StochasticLib3::WalleniusNCHypTable(int32_t n, int32_t m, int32_t N,
                                            double odds) {
  // Sampling from Wallenius noncentral hypergeometric distribution
  ...
  if (n != wnc_n_last || m != wnc_m_last || N != wnc_N_last ||
      odds != wnc_o_last) {
    ...
    CWalleniusNCHypergeometric wnch(n, m, N,
                                    odds); // make object for calculation
    success = wnch.MakeTable(wall_ytable, WALL_TABLELENGTH, &wall_x1,
                             &x2); // make table of probability values
    if (success) {
  ...
  while (1) {                           // repeat in the rare case of failure
    ...
    for (x = 0; x < wall_tablen; x++) { // chop-down search
      u -= wall_ytable[x];
      if (u < 0.)
        return x + wall_x1; // value found
    }
  }
}

int32_t StochasticLib3::WalleniusNCHypRatioOfUnifoms(int32_t n, int32_t m,
  ...
  while (1) { // rejection loop
    ...
    // rejection in t-domain implemented in
    // CWalleniusNCHypergeometric::BernouilliH
...
int32_t StochasticLib3::WalleniusNCHypInversion(int32_t n, int32_t m, int32_t N,
                                                double odds) {
  // sampling from Wallenius noncentral hypergeometric distribution
...
void StochasticLib3::MultiWalleniusNCHyp(int32_t *destination, int32_t *source,
  ...
  */
  ...
  int order2[MAXCOLORS]; // corresponding index into arrays when equal weights
                         // pooled together
  int order3[MAXCOLORS]; // secondary index for sorting by variance
  int32_t osource[MAXCOLORS]; // contents of source, sorted by weight with equal
                              // weights pooled together
  ...
  if (n < 0 || colors < 0 || colors > MAXCOLORS)
    FatalError("Parameter out of range in function MultiWalleniusNCHyp");
  if (colors == 0)
    return;
  if (n == 0) {
    for (i = 0; i < colors; i++)
  ...
  for (i = 0, msum = 0; i < colors; i++) {
    ...
    w = weights[i];
    if (m < 0 || w < 0)
      FatalError("Parameter negative in function MultiWalleniusNCHyp");
    if (w)
      msum += m;
  }
  N = msum;
  ...
  if (n >= N) {
    if (n > N)
      FatalError(
          "Taking more items than there are in function MultiWalleniusNCHyp");
    for (i = 0; i < colors; i++) {
      c = order1[i];
  ...
  if (colors2 < 3) {
    ...
    if (nn < 5000 * colors2) {
      ...
      do {
        ...
        if (urn[i] == 0) {
          if (i != j) {
            // put exhausted color at the end of lists so that colors2 can be
            // reduced
            m = osource[i];
            osource[i] = osource[j];
            osource[j] = m;
          ...
          if (colors2 == 2 && nn > 50) {
            ...
            x = WalleniusNCHyp(nn, urn[0], urn[0] + urn[1],
                               oweights[0] / oweights[1]);
            osample[0] += x;
            osample[1] += nn - x;
            break;
          }
      ...
      for (k = 0; k < 2; k++) {
        ...
        // split group into single colors by calling univariate distribution
        // b-a-1 times
        for (i = a; i < b - 1; i++) {
          m = urn[i];
          w = oweights[i];
      ...
      for (i = 0; i < colors; i++) {
        ...
        if (r1 <= 0. || r2 <= 0.) {
          ...
          var[i] = N * r1 * r2 /
                   ((N - 1) * (osource[i] * r2 + (N - osource[i]) * r1));
        }
      }
      ...
      for (k = 0; k < nHastings; k++) {
        for (i = 0; i < colors2; i++) {
          ...
          if (x == x0)
            continue; // accepted
          if (f0 < 0.)
            f0 = wmnc.probability(osample);
          CWalleniusNCHypergeometric nc(n1, osource[c1],
                                        osource[c1] + osource[c2], w, accuracy);
...
void StochasticLib3::MultiComplWalleniusNCHyp(int32_t *destination,
                                              int32_t *source, double *weights,
  ...
  for (i = 0, N = 0; i < colors; i++) {
    w = weights[i];
    if (w == 0)
      FatalError("Zero weight in function MultiComplWalleniusNCHyp");
    rweights[i] = 1. / w;
    N += source[i];
  }
...
******************************************************************************/
int32_t StochasticLib3::FishersNCHyp(int32_t n, int32_t m, int32_t N,
                                     double odds) {
  /*
  ...
  */
  ...
  if (n > N || m > N || n < 0 || m < 0 || odds <= 0.) {
    if (odds == 0.) {
      if (n > N - m)
        FatalError(
            "Not enough items with nonzero weight in function FishersNCHyp");
      return 0;
    }
...
int32_t StochasticLib3::FishersNCHypInversion(int32_t n, int32_t m, int32_t N,
                                              double odds) {
  /*
  ...
  */
  ...
  if (n != fnc_n_last || m != fnc_m_last || N != fnc_N_last ||
      odds != fnc_o_last) {
...
int32_t StochasticLib3::FishersNCHypRatioOfUnifoms(int32_t n, int32_t m,
                                                   int32_t N, double odds) {
  /*
  Subfunction for FishersNCHyp distribution.
...
***********************************************************************/
void StochasticLib3::MultiFishersNCHyp(int32_t *destination, int32_t *source,
  ...
  */
  int order1[MAXCOLORS]; // sort order, index into source and destination
  int order2[MAXCOLORS]; // corresponding index into osource when equal weights
                         // pooled together
  int order3[MAXCOLORS]; // secondary index for sorting by variance
  int32_t osource[MAXCOLORS]; // contents of source, sorted by weight with equal
                              // weights pooled together
  ...
  if (n >= msum) {
    if (n > msum)
      FatalError(
          "Taking more items than there are in function MultiFishersNCHyp");
    for (i = 0; i < colors; i++) {
      c = order1[i];
      destination[c] = source[c];
    }
  ...
  // copy source and weights into ordered lists and pool together colors with
  // same weight
  for (i = 0, c2 = -1; i < colors; i++) {
    c = order1[i];
    if (i == 0 || weights[c] != w) {
  ...
  if (colors2 > 2) {
    ...
    a = 0; // heavy group goes from a to b-1, light group goes from b to
           // colors2-1
    ...
    for (k = 0; k < ngibbs; k++) {
      for (i = 0; i < colors2; i++) {
        ...
        c2 = order3[j];
        n1 = osample[c1] + osample[c2];
        x = FishersNCHyp(n1, osource[c1], osource[c1] + osource[c2],
                         oweights[c1] / oweights[c2]);
        osample[c1] = x;
        osample[c2] = n1 - x;
      }
    }
