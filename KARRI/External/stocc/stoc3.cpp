...
Constructor
***********************************************************************/
StochasticLib3::StochasticLib3(int seed)
    : StochasticLib1(seed)
{
...
int32_t StochasticLib3::WalleniusNCHyp(int32_t n, int32_t m, int32_t N, double odds)
{
    ...
    */
    ...
    if (n >= N || m >= N || n <= 0 || m <= 0 || odds <= 0.) {
        ...
        if (odds == 0.) {
            if (n > N - m)
                FatalError("Not enough items with nonzero weight in function WalleniusNCHyp");
            return 0;
        }
        // illegal parameter
    ...
    int32_t* source, double* weights, int32_t n, int colors)
{
    ...
    */
    ...
    if (n >= N) {
        if (n > N)
            FatalError("Taking more items than there are in function MultiWalleniusNCHyp");
        for (i = 0; i < colors; i++) {
            c = order1[i];
...
int32_t StochasticLib3::FishersNCHyp(int32_t n, int32_t m, int32_t N, double odds)
{
    ...
    */
    ...
    if (n > N || m > N || n < 0 || m < 0 || odds <= 0.) {
        if (odds == 0.) {
            if (n > N - m)
                FatalError("Not enough items with nonzero weight in function FishersNCHyp");
            return 0;
        }
