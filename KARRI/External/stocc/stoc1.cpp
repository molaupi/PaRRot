...
int32_t StochasticLib1::HypInversionMod(int32_t n, int32_t m, int32_t N) {
  ...
  */
  ...
  if (N != hyp_N_last || m != hyp_m_last || n != hyp_n_last) {
    ...
    hyp_bound =
        (int32_t)(modef +
                  11. * sqrt(modef * (1. - p) * (1. - n / (double)N) + 1.));
    if (hyp_bound > n)
      hyp_bound = n;
  }
...
double StochasticLib1::fc_lnpk(int32_t k, int32_t L, int32_t m, int32_t n) {
  // subfunction used by hypergeometric and Fisher's noncentral hypergeometric
  // distribution
...
double StochasticLib1::Normal(double m, double s) {
  ...
  w = sqrt(log(w) * (-2. / w));
  normal_x1 *= w;
  normal_x2 *= w;      // normal_x1 and normal_x2 are independent normally
...
double StochasticLib1::NormalTrunc(double m, double s, double limit) {
  ...
  if (limit < s)
    FatalError("limit out of range in NormalTrunc function");
  double x;
  do {
    x = Normal(0., s);
