  ...
  */
  ...
  for (;;) {
    ...
    if ((U = Random() * Bino_p6) < Bino_p2) { // centre left
      // immediate acceptance region R2 = [k2, mode) *[0, f2),  X = k2, ... mode
      // -1
      if ((V = U - Bino_p1) < 0.)
        return (Bino_k2 + (int32_t)(U / Bino_f2));
      ...
      if ((V = Bino_f2 + Bino_f2 - W) < 1.) { // quick reject of Y
        Y = Bino_k2 + Dk;
        if (V <=
            Bino_f2 + Dk * (1. - Bino_f2) / (Bino_dl + 1.)) { // quick accept of
          return (Y);
        } // Y = k2 + Dk
    ...
    // log BinomialF(X) = (X - mode)*log(p/q) - log X!(n - X)! + log mode!(n -
    // mode)!
...
int32_t StochasticLib2::PoissonModeSearch(double L) {
  ...
  */
  ...
  if (L != pois_L_last) { // set up
    pois_L_last = L;
    pois_bound = (int32_t)floor(L + 0.5 +
                                7.0 * (sqrt(L + L + 1.) + 1.5)); // safety-bound
  ...
  */
  ...
  for (;;) {
    ...
    if ((U = Random() * Pois_p6) < Pois_p2) { // centre left
      ...
      // -1
      if ((V = U - Pois_p1) < 0.0)
        return (Pois_k2 + (int32_t)(U / Pois_f2));
      // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
      ...
      //  immediate acceptance region R3 = [mode, k4+1)*[0, f4), X = mode, ...
      //  k4
      if ((V = U - Pois_p3) < 0.)
    ...
    if (log(W) <= X * Pois_l_my - LnFac(X) - Pois_c_pm)
      return X;
  }
}
  ...
  */
  ...
  while (1) {
    ...
    if ((U = Random() * Hyp_p6) < Hyp_p2) { // centre left
      ...
      // -1
      if ((W = U - Hyp_p1) < 0.)
        return (Hyp_k2 + (int32_t)(U / Hyp_f2));
      // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
      if ((Y = W / Hyp_dl) < Hyp_f1)
        return (Hyp_k1 + (int32_t)(W / Hyp_f1));
      ...
      // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
      if ((Y = W / Hyp_dr) < Hyp_f5)
        return (Hyp_k5 - (int32_t)(W / Hyp_f5));
    ...
    if (log(Y) <= Hyp_c_pm - fc_lnpk(X, Hyp_L, m, n))
      return (X);
  }
}
