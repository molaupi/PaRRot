...
CFishersNCHypergeometric::CFishersNCHypergeometric(int32_t n, int32_t m,
...
double CFishersNCHypergeometric::probability(int32_t x) {
  ...
  if (odds == 1.) {
    ...
    return exp(LnFac(m) - LnFac(x) - LnFac(m - x) + LnFac(N - m) -
               LnFac(n - x) - LnFac((N - m) - (n - x)) -
  ...
  if (odds == 0.) {
    if (n > N - m)
      FatalError("Not enough items with nonzero weight in "
                 "CFishersNCHypergeometric::probability");
    return x == 0;
  }
...
double CFishersNCHypergeometric::probabilityRatio(int32_t x, int32_t x0) {
  ...
  if (dx <= 28 && x <= 100000) { // avoid overflow
    ...
    f1 = FallingFactorial(a1, dx) + FallingFactorial(a2, dx) -
         FallingFactorial(a3, dx) - FallingFactorial(a4, dx) + dx * log(odds);
    if (invert)
      f1 = -f1;
    f1 = exp(f1);
  }
...
double CFishersNCHypergeometric::MakeTable(double *table, int32_t MaxLength,
  ...
  if (MaxLength <= 0) {
    ...
    if (xfirst)
      *xfirst = 1; // for analogy with CWalleniusNCHypergeometric::MakeTable
  ...
  if (mode - x1 <= MaxLength / 2) {
    ...
    // There is not enough space for any of the tails. Place mode in middle of
    // table
    i0 = MaxLength / 2;
  }
  // Table start index
  ...
  for (i = i0 - 1; i >= i1; i--) {
    ...
    if (f < cutoff) {
      i1 = i;
      break; // cut off tail if < accuracy
    }
  }
  if (i1 > 0) {
...
CMultiFishersNCHypergeometric::CMultiFishersNCHypergeometric(
  ...
  if (N < n)
    FatalError(
        "Not enough items in constructor for CMultiFishersNCHypergeometric");
  if (N1 < n)
...
void CMultiFishersNCHypergeometric::mean(double *mu) {
  ...
  if (colors < 3) {
    ...
    if (colors == 1)
      mu[0] = n;
    if (colors == 2) {
      mu[0] = CFishersNCHypergeometric(n, m[0], m[0] + m[1], odds[0] / odds[1])
                  .mean();
      mu[1] = n - mu[0];
    }
    return;
  ...
  do {
    ...
    r *= n * (N - q) / (q * (N - n));
    if (++iter > 100)
      FatalError("convergence problem in function "
...
double CMultiFishersNCHypergeometric::probability(int32_t *x) {
  ...
  if (xsum != n) {
    FatalError("sum of x values not equal to n in function "
               "CMultiFishersNCHypergeometric::probability");
  }
  ...
  if (sn == 0)
    SumOfAll();              // first time initialize
  return exp(lng(x)) * rsum; // function value
}

double CMultiFishersNCHypergeometric::moments(double *mean, double *variance,
